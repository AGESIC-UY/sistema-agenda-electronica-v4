...
public class DataTableRenderer extends DataRenderer {
    ...
    private static final String DEFAULT_CAPTION = "Table caption";
    private static final String DEFAULT_CAPTION_CLASS = "ui-helper-hidden-accessible";
    ...
    protected void encodeRegularTable(FacesContext context, DataTable table) throws IOException {
        ...
        //Sofis: es obligatorio que haya un caption para cada tabla
        writer.startElement("caption", null);
        if (!ComponentUtils.isValueBlank(table.getCaptionClass())) {
            writer.writeAttribute("class", table.getCaptionClass(), null);
        } else {
            writer.writeAttribute("class", DEFAULT_CAPTION_CLASS, null);
        }
        if (!ComponentUtils.isValueBlank(table.getCaption())) {
            writer.write(table.getCaption());
        } else {
            writer.write(DEFAULT_CAPTION);
        }
    ...
    public void encodeColumnHeader(FacesContext context, DataTable table, UIColumn column) throws IOException {
        ...
        if (selectionMode != null && selectionMode.equalsIgnoreCase("multiple")) {
            encodeCheckbox(context, table, false, false, HTML.CHECKBOX_ALL_CLASS, true, null);
        }
    ...
    protected void encodeCell(FacesContext context, DataTable table, UIColumn column, String clientId, boolean selected) throws IOException {
        ...
        if (selectionEnabled) {
          ...
          //Sofis: esto estaba sin else, pero debido a la necesidad de utilizar un OutputLabel para 
          //especificar la label correspondiente al checkbox o radio button que se usa para la
          //selección se determinó que si la columna es de selección entonces no se renderiza
          //ninguno de los componentes indicados y solo se utiliza un OutputLabel (si hay) para
          //generar el componente label del radio o checkbox
    ...
    protected void encodeStateHolder(FacesContext context, DataTable table, String id, String value) throws IOException {
        ...
        //Sofis: el atributo autocomplete no es valido segun el estandar
        //writer.writeAttribute("autocomplete", "off", null);
        if (value != null) {
    ...
    protected void encodeColumnSelection(FacesContext context, DataTable table, String clientId, UIColumn column, boolean selected)
        ...
        String selectionMode = column.getSelectionMode();
        String radioClientId = column.getClientId(context);
        boolean disabled = table.isDisabledSelection();

        if (selectionMode.equalsIgnoreCase("single")) {
            //Sofis: si la columna tiene un hijo "HtmlOutputLabel" se usa como label para el radio button
            //(por accesibilidad todos los componentes deben tener label)
            encodeRadio(context, table, selected, disabled, radioClientId);
              ...
              for(UIComponent child : column.getChildren()) {
                if(child instanceof HtmlOutputLabel) {
                  ...
                  if(label.getValue()!=null) {
                    text = label.getValue().toString();
                    ResponseWriter writer = context.getResponseWriter();
                    writer.startElement("label", null);
                    writer.writeAttribute("for", radioClientId, "for");
                    writer.writeAttribute("class", "ui-helper-hidden-accessible clase2", "class");
                    writer.write(text);
                    writer.endElement("label");
                  }
                }
            ...
            encodeCheckbox(context, table, selected, disabled, HTML.CHECKBOX_CLASS, false, radioClientId);
    ...
    protected void encodeRadio(FacesContext context, DataTable table, boolean checked, boolean disabled, String clientId) throws IOException {
        ResponseWriter writer = context.getResponseWriter();

        if (table.isNativeElements()) {
            encodeNativeRadio(context, table, checked, disabled, clientId);
        } else {
    ...
    protected void encodeCheckbox(FacesContext context, DataTable table, boolean checked, boolean disabled, String styleClass,
            boolean isHeaderCheckbox, String clientId) throws IOException {

        ResponseWriter writer = context.getResponseWriter();

        if (table.isNativeElements()) {
            encodeNativeCheckbox(context, table, checked, disabled, isHeaderCheckbox, clientId);
        } else {
    ...
    protected void encodeNativeCheckbox(FacesContext context, DataTable table, boolean checked, boolean disabled,
        ...
        //Sofis: para que se pueda poner un label a los checkboxes
        if(clientId != null) {
          writer.writeAttribute("id", clientId, null);
        }
    ...
    protected void encodeNativeRadio(FacesContext context, DataTable table, boolean checked, boolean disabled, String clientId) throws IOException {
        ResponseWriter writer = context.getResponseWriter();

        writer.startElement("input", null);
        writer.writeAttribute("type", "radio", null);
        //Sofis: para que se pueda poner un label a los radiobutton
        if(clientId != null) {
          writer.writeAttribute("id", clientId, null);
        }
