...
@Stateless
public class ConsultasBean implements ConsultasLocal {
    ...
    private static final int MAXIMO_PERSONAS = 15;

    /**
     * Obtiene una reserva por su identificador. No toma en cuenta si la reserva
     * corresponde o no a una disponibilidad presencial.
     */
    ...
    /**
     * Obtiene una reserva por la combinación de fecha/hora/número. No toma en
     * cuenta a las reservas que corresponden a disponibilidades presenciales.
     */
    ...
    /**
     * Busca las reservas confirmadas existentes que tengan los mismos valores
     * en todos los datos claves y que correspondan al trámite indicado. No
     * considera reservas que corresponden a disponibilidades presenciales.
     *
     * @param datos
     * @param recurso
     * @param fecha
    ...
    /**
     * Busca reservas en base a los datos especificados para ser usada para la
     * cancelación. No toma en cuenta las reservas correspondientes a atenciones
     * presenciales.
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Busca las reservas de un recurso para un período de tiempo según el
     * estado.
     */
    ...
    /**
     * Busca las reservas de un recurso para un período de tiempo que estén en
     * el estado Reservado (esperando ser llamadas por primera vez).
     */
    ...
    /**
     * Busca las reservas de un recurso para un período de tiempo que estén en
     * el estado Reservado o Usado (esperando ser llamadas).
     */
    ...
    /**
     * Consulta las disponibilidades que tienen alguna reserva para el período
     * indicado. Si el parámetro atencionPresencial es null se ignora, sino se
     * busca que la disponibilidad coincida con ese parámetro
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Esta función es utilizada por el servicio web REST para determinar las
     * fechas de las reservas que tiene una persona identificada por su tipo y
     * número de documento en una agenda y recurso especial; la empresa se
     * determina en base al token, el cual debe estar registrado (el organismo
     * que desee invocar el servicio tendrá que solicitar un token para cada
     * empresa). No considera las reservas correspondientes a disponibilidades
     * presenciales.
     */
    ...
    /**
     * Esta función es utilizada por el servicio web REST para determinar las
     * reservas vigentes que tiene una lista de personas identificadas por su
     * tipo y número de documento; la empresa se determina en base al token, el
     * cual debe estar registrado (el organismo que desee invocar el servicio
     * tendrá que solicitar un token para cada empresa). No considera las
     * reservas correspondientes a disponibilidades presenciales.
     */
    public List<Map<String, Object>> consultarReservasVigentesPorTokenYEmpresaTramiteDocumento(String token, String idEmpresa, String codTramite, List<String[]> personas) throws UserException {
        if (personas.size() > MAXIMO_PERSONAS) {
            throw new UserException("maximo_permitido_cantidad_personas_" + MAXIMO_PERSONAS);
        }
        List<Map<String, Object>> respuesta = new ArrayList<>();
        for (String[] persona : personas) {
            respuesta.addAll(consultarReservasVigentesPorTokenYEmpresaporCadaUno(token, idEmpresa, codTramite, persona[0], persona[1]));
        }
        return respuesta;
    }

    private List<Map<String, Object>> consultarReservasVigentesPorTokenYEmpresaporCadaUno(String token, String idEmpresa, String codTramite, String tipoDoc, String numDoc) throws UserException {

        if (tipoDoc == null || tipoDoc.trim().length() == 0 || numDoc == null || numDoc.trim().length() == 0) {
        ...
        try {
            ...
            query = "SELECT res.id, res.estado, res.serie, res.numero, res.codigo_seguridad, res.trazabilidad_guid, res.tramite_codigo, res.tramite_nombre,"
                    + " dis.hora_inicio, dis.presencial, rec.nombre as nombre_recurso, age.nombre as agenda_recurso, ate.funcionario, ate.asistio, ds.etiqueta, dr.valor"
                    + ", age.id as age_id, rec.id as rec_id"//Agregado 16 y 17
                    + " FROM {esquema}.ae_reservas res "
                    + " JOIN {esquema}.ae_reservas_disponibilidades rd ON rd.aers_id=res.id "
                    + " JOIN {esquema}.ae_disponibilidades dis ON dis.id=rd.aedi_id "
                    + " JOIN {esquema}.ae_recursos rec ON rec.id=dis.aere_id "
            ...
            query = query + "   WHERE res.estado=:estado";
            ...
            //query = query + "   AND DATE_TRUNC('minute',dis.hora_fin)>=DATE_TRUNC('minute',now()) ";
            query = query + " ORDER BY res.id";
            query = query.replace("{esquema}", esquema);
            ...
            for (Object[] res : ress) {
                ...
                if (reservaIdActual == null || !reservaIdActual.equals(reservaId)) {
                    reservaIdActual = reservaId;
                    reserva = new HashMap<String, Object>();
                    reserva.put("tipoDocumento", tipoDoc);//Agregado
                    reserva.put("numeroDocumento", numDoc);//Agregado
                    reserva.put("reservaId", reservaId);
                    reserva.put("estado", res[1]);
                    reserva.put("serie", res[2]);
                    reserva.put("numero", res[3]);
                ...
                //Agregado
                reserva.put("agendaId", res[16]);
                reserva.put("recursoId", res[17]);
            }
            return resp;
        } catch (NoResultException nrEx) {
            throw new UserException("no_se_encuentra_el_token_especificado");
    ...
    /**
     * Esta función es utilizada por el servicio web REST para determinar las
     * reservas vigentes que tiene una persona identificada por su tipo y número
     * de documento en una agenda (y recurso opcional); la empresa se determina
     * en base al token, el cual debe estar registrado (el organismo que desee
     * invocar el servicio tendrá que solicitar un token para cada empresa). No
     * considera las reservas correspondientes a disponibilidades presenciales.
     */
    ...
    /**
     * Permite obtener los recursos para una agenda, según los requerimientos de
     * la operación "recursos_por_agenda" del servicio web REST
     */
    public Map<String, Object> consultarRecursosPorAgenda(Integer idEmpresa, Integer idAgenda, String idioma) throws UserException {
        ...
        try {
            ...
            } else if (!agenda.getTextosAgenda().isEmpty()) {
            ...
            for (Recurso recurso : recursos) {
                if (recurso.getVisibleInternet()) {
                    ...
                    rec.put("longitud", recurso.getLongitud() != null ? recurso.getLongitud().toString() : "");
            ...
            LOGGER.log(Level.SEVERE, "No se pudo consultar la lista de recursos por agenda", nrEx);
            throw new UserException("error_no_solucionable");
        }
    }

    /**
     * Permite consultar las disponibilidades para un recurso, según los
     * requerimientos de la operación "disponibilidades_por_recurso" del
     * servicio web REST
     */
    public Map<String, Object> consultarDisponibilidadesPorRecurso(Integer idEmpresa, Integer idAgenda, Integer idRecurso, String idioma) throws UserException {
        ...
        try {
            ...
            } else if (!recurso.getTextosRecurso().isEmpty()) {
    ...
    public List<ReservaDTO> consultarReservasCanceladas(Recurso recurso, String codigoTramite, Date reservaFechaDesde, Date reservaFechaHasta,
        ...
        if (creacionFechaDesde != null) {
            consulta = consulta + " AND r.fechaCreacion >= :creacionFechaDesde";
        }
        if (creacionFechaHasta != null) {
            consulta = consulta + " AND r.fechaCreacion <= :creacionFechaHasta";
        }
        if (cancelacionFechaDesde != null) {
            consulta = consulta + " AND r.fcancela >= :cancelacionFechaDesde";
        }
        if (cancelacionFechaHasta != null) {
            consulta = consulta + " AND r.fcancela <= :cancelacionFechaHasta";
        }
        ...
        if (reservaFechaHasta != null) {
        	...
        	calendar.setTime(reservaFechaHasta);
        	calendar.add(Calendar.HOUR, 23);
        	calendar.add(Calendar.MINUTE, 59);
        	calendar.add(Calendar.SECOND, 59);
        	reservaFechaHasta = calendar.getTime();
        ...
        if (creacionFechaHasta != null) {
        	Calendar calendar = Calendar.getInstance();
        	calendar.setTime(creacionFechaHasta);
        	calendar.add(Calendar.DAY_OF_YEAR, 1);
        	creacionFechaHasta = calendar.getTime();
        ...
        if (cancelacionFechaHasta != null) {
        	Calendar calendar = Calendar.getInstance();
        	calendar.setTime(cancelacionFechaHasta);
        	calendar.add(Calendar.DAY_OF_YEAR, 1);        	
        	cancelacionFechaHasta = calendar.getTime();
    ...
    /**
     * Esta función es utilizada por el servicio web REST para determinar los
     * cambios en los 'Datos generales' de un recurso durante un periodo
     * determinado.La empresa se determina en base al token, el cual debe estar
     * registrado (el organismo que desee invocar el servicio tendrá que
     * solicitar un token para cada empresa).
     *
     * @param token
     * @param idEmpresa
     * @param idAgenda
     ...
     * @param fechaHasta
     * @return
     * @throws uy.gub.imm.sae.exception.UserException
     */
    @Override
    public List<RecursoDTO> consultarCambiosEnUnRecurso(String token, Integer idEmpresa, Integer idAgenda, Integer idRecurso, LocalDateTime fechaDesde, LocalDateTime fechaHasta) throws UserException {
        String salidaError = "";
        try {
            if (token.isEmpty()) {
                salidaError = "debe_especificar_el_token";
                throw new UserException(salidaError);
            }
            if (idEmpresa == null) {
                salidaError = "debe_especificar_la_empresa";
                throw new UserException(salidaError);
            }
            if (idAgenda == null) {
                salidaError = "debe_especificar_la_agenda";
                throw new UserException(salidaError);
            }
            if (idRecurso == null) {
                salidaError = "debe_especificar_el_recurso";
                throw new UserException(salidaError);
            }
            //Obtener la empresa
            Empresa empresa;
            empresa = empresasEJB.obtenerEmpresaPorId(idEmpresa);
            if (empresa == null) {
                salidaError = "no_se_encuentra_la_empresa_especificada";
                throw new UserException(salidaError);
            }

            //Validar el token y la empresa
            boolean tokenValido = this.validarTokenEmpresa(token, idEmpresa);
            if (!tokenValido) {
                salidaError = "no_se_encuentra_el_token_especificado_para_esta_empresa";
                throw new UserException(salidaError);
            }
            ...
            //Obtener la agenda
            String queryAgenda = "SELECT ag.id FROM {esquema}.ae_agendas ag WHERE ag.id=:idAgenda";
            queryAgenda = queryAgenda.replace("{esquema}", esquema);
            Query qAgenda = globalEntityManager.createNativeQuery(queryAgenda);
            qAgenda.setParameter("idAgenda", idAgenda);

            if (qAgenda.getResultList().isEmpty()) {
                salidaError = "no_se_encuentra_la_agenda_especificada";
                throw new UserException(salidaError);
            }

            //Obtener el recurso
            String queryRecurso = "SELECT rec.id FROM {esquema}.ae_recursos rec"
                    + " JOIN {esquema}.ae_agendas ag ON rec.aeag_id=ag.id "
                    + " WHERE rec.id=:idRecurso AND ag.id=:idAgenda";

            queryRecurso = queryRecurso.replace("{esquema}", esquema);
            Query qRecurso = globalEntityManager.createNativeQuery(queryRecurso);
            qRecurso.setParameter("idRecurso", idRecurso);
            qRecurso.setParameter("idAgenda", idAgenda);

            if (qRecurso.getResultList().isEmpty()) {
                salidaError = "no_se_encuentra_el_recurso_especificado";
                throw new UserException(salidaError);
            }
            ...
            if (fechaDesde != null) {
                query1.setParameter("fechaDesde",
                    Date.from(fechaDesde.atZone(ZoneId.systemDefault()).toInstant()),
                    TemporalType.TIMESTAMP);
            }

            if (fechaHasta != null) {
                query1.setParameter("fechaHasta",
                    Date.from(fechaHasta.atZone(ZoneId.systemDefault()).toInstant()),
                    TemporalType.TIMESTAMP);
            }
