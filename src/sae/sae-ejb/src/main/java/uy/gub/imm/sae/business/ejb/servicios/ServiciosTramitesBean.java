...
//    }
    ...
    @SuppressWarnings({"unchecked", "rawtypes"})
    public List<UnidadEjecutora> obtenerUnidadesEjecutoras(Integer organismoId) throws ApplicationException {
        try {
            logger.info("Obteniendo unidades ejecutoras de TrámitesUy...");

            URL urlWsdl = WsTramite.class.getResource("WsTramite.wsdl");

            String wsUser = confBean.getString("WS_TRAMITE_USER",organismoId);
            String wsPass = confBean.getString("WS_TRAMITE_PASS",organismoId);
            ...
            //Configurar la URL del servicio web
            String wsAddressLocation = confBean.getString("WS_TRAMITE_LOCATION_INFO",organismoId);
            if (wsAddressLocation != null) {
                //Si no se define se deja lo que tenga el WSDL
                bindingProvider.getRequestContext().put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY, wsAddressLocation);
            }
            ...
            Long timeout = 5000L;
            try {
                timeout = confBean.getLong("WS_TRAMITE_TIMEOUT",organismoId);
                if (timeout == null) {
                    timeout = 5000L;
                }
    ...
    @SuppressWarnings({"unchecked", "rawtypes"})
    public List<Tramite> obtenerTramitesOrganismoYUnidadEjecutora(Integer empresaId, Integer organismoCod, Integer unidadEjCod, Integer organismoId) throws ApplicationException, UserException {
        try {
            ...
            //Configurar la URL del servicio web
            String wsAddressLocation = confBean.getString("WS_TRAMITE_LOCATION_GUIA",organismoId);
            if (wsAddressLocation != null) {
                //Si no se define se deja lo que tenga el WSDL
                bindingProvider.getRequestContext().put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY, wsAddressLocation);
            }
    ...
    public List<Oficina> obtenerOficinasTramite(Tramite tramite, Integer organismoId) throws ApplicationException {
        try {
