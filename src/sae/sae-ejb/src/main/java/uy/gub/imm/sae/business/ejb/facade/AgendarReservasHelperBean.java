...
public class AgendarReservasHelperBean implements AgendarReservasHelperLocal {
    ...
    private static final Logger LOGGER = Logger.getLogger(AgendarReservasHelperBean.class);
    ...
    /**
     * Obtiene la ventana del calendario estatica, es decir sin verificar los cupos que realmente existen en la ventana.
     *
     * @param recurso
     * @return
     */
    ...
    public VentanaDeTiempo obtenerVentanaCalendarioEstaticaIntranet(Recurso recurso) {
        ...
        cal.setTime(hoy);
        try {
            for (int i = 0; i < recurso.getDiasInicioVentanaIntranet();) {
    ...
    public VentanaDeTiempo obtenerVentanaCalendarioEstaticaInternet(Recurso recurso) {
        ...
        cal.setTime(hoy);
        try {
            for (int i = 0; i < recurso.getDiasInicioVentanaInternet();) {
                if (Calendario.esDiaHabil(cal.getTime(), recurso)) {
                    i++;
                }
                cal.add(Calendar.DAY_OF_MONTH, 1);
    ...
    /*
     * Obtiene los cupos asignados, es decir la suma de los cupos de las disponibilidades existentes no nulas dentro de la ventana indicada. Para el rango de la
     * ventana que caiga en el pasado o fuera del inicio de disponibilidad indicado en el recurso, no se devuelve cupos.
     */
    ...
    /*
     * Obtiene los cupos consumidos, es decir la suma de las reservas no canceladas dentro de la ventana indicada. Para el rango de la ventana que caiga en el
     * pasado o fuera del inicio de disponibilidad indicado en el recurso, no se devuelve cupos.
     */
    @SuppressWarnings("unchecked")
    @Override
    ...
    public List<Integer> obtenerCuposXDia(VentanaDeTiempo v, List<Object[]> cuposAsignados, List<Object[]> cuposConsumidos) {
        ...
        List<Integer> cuposXdia = new ArrayList<>();
    ...
    @Override
    public Reserva crearReservaPendiente(Disponibilidad disp, OrigenReserva origen, TokenReserva token, String ipOrigen) {
    ...
    /*
     * Verifica que existan cupos disponibles para la reserva. Si el parámetro reservaTomada es true asume que pueden quedar 0 reservas porque el llamador ya
     * tomó la reserva (la última) Si el parámetro reservaTomada es false deben quedar al menos 1 cupo para que el llamador pueda tomarlo Si la reserva está
     * cancelada hay que tomar en cuenta si ya se liberó el cupo
     */
    @Override
    ...
    public List<Reserva> validarDatosReservaPorClave(Recurso recurso, Disponibilidad disponibilidad,
        ...
        List<DatoASolicitar> camposClave = new ArrayList<>();
        ...
        if (!camposClave.isEmpty()) {
            ...
            Date fechaDesde = Utiles.time2InicioDelDia(cal.getTime());
            cal.setTime(fecha);
            cal.add(Calendar.DATE, 1 * periodoValidacion);
            Date fechaHasta = Utiles.time2FinDelDia(cal.getTime());
    ...
    public void validarDatosReservaExtendido(List<ValidacionPorRecurso> validaciones, List<DatoASolicitar> campos,
        ...
        Map<String, String> datosClave = new HashMap<>();
    ...
    private ValidadorReserva getValidador(String host, String jndiName) throws ApplicationException {
        ...
        try {
            ...
            LOGGER.error("No se pudo acceder a un EJB de tipo ValidadorReserva (jndiName: " + jndiName + ")", nEx);
            throw new ApplicationException("No se pudo acceder a un EJB de tipo ValidadorReserva (jndiName: " + jndiName + ")", nEx);
        }
        if (ejb instanceof ValidadorReserva) {
            return (ValidadorReserva) ejb;
        } else {
            LOGGER.error("Se esperaba un EJB de tipo ValidadorReserva y se encontró uno del tipo " + ejb.getClass());
    ...
    public Map<String, Object> autocompletarCampo(ServicioPorRecurso s, Map<String, Object> datosParam) throws ApplicationException, BusinessException, ErrorAutocompletarException, WarningAutocompletarException {
        ...
        Map<String, Object> paramEntradaMap = new HashMap<>();
        Map<String, Object> paramSalidaMap = new HashMap<>();
        List<String> nombreCampos = new ArrayList<>();
        ...
        try {
            ...
            if (resultado.getWarnings().size() > 0) {
                List<String> mensajes = new ArrayList<>();
                List<String> codigosWarningMensajes = new ArrayList<>();
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    @Override
    public Reserva crearReservasParesPendiente(Disponibilidad disp,Disponibilidad disp2, OrigenReserva origen, TokenReserva token, String ipOrigen) throws ApplicationException {
        Reserva reserva = new Reserva();
        disp = entityManager.find(Disponibilidad.class, disp.getId());
