...
import uy.gub.imm.sae.business.utilidades.Calendario;
...
@Stateless
public class DisponibilidadesBean implements DisponibilidadesLocal {
    ...
    private static final Logger LOGGER = Logger.getLogger(DisponibilidadesBean.class);
    ...
    public Disponibilidad obtenerDisponibilidadPorId(Integer id) {
        ...
        try {
            return (Disponibilidad) entityManager.find(Disponibilidad.class, id);
        } catch (NoResultException nrEx) {
            return null;
        } catch (Exception ex) {
            LOGGER.error("No se pudo obtener la disponibilidad por id", ex);
            return null;
        }
    }

    /*
     * Elmina las disponibilidades del recurso indicado que estén dentro de la ventana especificada No toma en cuenta las disponibilidades presenciales
     */
    ...
    /**
     * Genera disponibilidades en una fecha para un recurso. Controla: 1) Que no exista una disponibilidad viva para la misma fecha y la misma hora. 2) Solo se
     * generen disponibilidades para días que se encuentren marcados como hábiles en sp_dias. 3) Si ya hay disponibilidades para alguna hora se registra y se
     * devuelve.
     *
    ...
    /**
     * Genera disponibilidades en una ventana de tiempo para un recurso. Toma como modelo las disponibilidades generadas para una fecha. Controla: 1) Los días a
     * generar se encuentren entre fechaInicioDisp y fechaFinDisp del recurso. 2) La cantidad de días a generar no puede superar cantDiasAGenerar del recurso.
     * 3) Solo se generen disponibilidades para días que se encuentren marcados como hábiles en sp_dias. El parámetro dias debe ser un array de 6 lugares
     * indicando para cada día si aplica o no (0=lunes,1=martes,2...,5=sábado)
     *
     * @throws UserException
     * @throws ApplicationException
     */
    @SuppressWarnings("unchecked")
    public List<Date> generarDisponibilidades(Recurso recurso, Date fechaModelo, VentanaDeTiempo v, Boolean[] dias) throws UserException {
        ...
        //Listas de conflictos de fechas
        List<Date> horasConflicto = new ArrayList<Date>();
        while (!cal.getTime().after(v.getFechaFinal())) {
            if (esDiaHabil(cal.getTime(), recurso) && considerarDia(cal, dias)) {
                for (Disponibilidad d : disponibilidades) {
                    ...
                    calAux.setTime(cal.getTime());
                    calTime.setTime(d.getHoraInicio());

                    calAux.set(Calendar.HOUR_OF_DAY, calTime.get(Calendar.HOUR_OF_DAY));
                    calAux.set(Calendar.MINUTE, calTime.get(Calendar.MINUTE));
                    calAux.set(Calendar.SECOND, calTime.get(Calendar.SECOND));
                    //Chequeamos que no existan disponibilidades solapadas en el dia destino
                    if (!existeDisponibilidadEnHoraInicio(recurso, calAux.getTime())) {
                        generarNuevaDisponibilidad(recurso, cal.getTime(), d.getHoraInicio(), d.getHoraFin(), d.getCupo());
                    } else {
                        //Registra la hora de conflicto
                        horasConflicto.add(calAux.getTime());
                    }
                }
            }
    ...
    /**
     * Genera una nueva disponibilidad. Se debe tener en cuenta que hora inicio y hora fin tienen también la fecha. Para que la agenda funcione correctamente la
     * fecha para ambos casos deberá ser igual que la fecha de la disponibilidad.
     */
    ...
    /**
     * Obtiene información sobre las disponibilidades y las reservas para cada día de la ventana especificada para el recurso indicado. No considera las
     * disponibilidades presenciales. Para las reservas canceladas toma en cuenta la fechaLiberacion
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Devuelve true si pudo hacer la modificación solicitada, o false en otro caso (por ejemplo, si el nuevo cupo es menor que la cantidad de reservas ya
     * hechas, se pone como cupo este último valor y no el solicitado)
     */
    ...
    /**
     * Determina la cantidad de reservas existentes para la disponibilidad indicada La disponibilidad puede o no ser presencial Esta operación NO toma en cuenta
    ...
    /**
     * Modifica el cupo de la fecha actual No considera las disponibilidades presenciales
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Modifica el cupo de la fecha actual y de todos los días posteriores para la hora indicada por la disponibilidad. La acción aplicada es la indicada
     * (aumentar, disminuir, establecer) y el valor el indicado. No considera las disponibilidades presenciales
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Determina cuántas disponibilidades hay para la fecha indicada No toma en cuenta las disponibilidades presenciales
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Retorna true si existen disponibilidades vivas para un recurso, fecha y hora de inicio. No considera las disponibilidades presenciales
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Devuelve true si la fecha indicada corresponde a un día habil Si sábado o domingo toma en cuenta la configuración del recurso
     */
    ...
    /**
     * Determina si se debe considerar la fecha indicada para aplicar algún procedimiento según el mapa de días especificado.
     *
     * @param cal
     * @param dias
     * @return
