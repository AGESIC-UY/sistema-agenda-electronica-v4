...
@Stateless
public class ValidacionesBean implements ValidacionesLocal {
    ...
    /*
     * Crea la validacion <b>v</b> en el sistema. Controla la unicidad del nombre de la validacion entre todas las validaciones vivas (fechaBaja == null).
     * Retorna la validacion con su identificador interno. Roles permitidos: Administrador
     *
    ...
    /*
     * Se realiza la baja lógica de la validacion (se setea fechaBaja con la fecha actual del sistema). Controla que no existan datos vivos asociados a la
     * validacion , si es así no se la da de baja.
     *
     *
     * @throws UserException
     * @throws ApplicationException
    ...
    /*
     * Se realiza la modificación de la validacion <b>v</b>. Controla la unicidad del nombre de la validacion entre todas las validaciones vivas (fechaBaja ==
     * null). Actualiza los parametros de acuerdo a los indicados en la validacion de entrada. Roles permitidos: Administrador
     *
     * @throws UserException
     * @throws ApplicationException
     */
    ...
    private void modificarParametrosValidacion(Validacion v, List<ParametroValidacion> parametrosModificados) throws UserException {
        ...
        for (ParametroValidacion pActual : parametrosActuales) {
            ...
            if (mapModificados.containsKey(pActual.getId())) {
                ...
                if (pModificado.getNombre() == null || pModificado.getNombre().equals("")) {
                    throw new UserException("-1", "Falta indicar el nombre de un parámetro");
                }
                if (pModificado.getNombre().length() > 50) {
                    throw new UserException("-1", "El nombre del parámetro es demasiado largo, máximo 50 caracteres.");
                }
        ...
        for (ParametroValidacion pModificado : parametrosModificados) {
            if (pModificado.getId() == null) {
                ...
                if (pNuevo.getNombre() == null || pNuevo.getNombre().equals("")) {
                    throw new UserException("-1", "Falta indicar el nombre de un parámetro");
                }
                if (pNuevo.getNombre().length() > 50) {
                    throw new UserException("-1", "El nombre del parámetro es demasiado largo, máximo 50 caracteres.");
                }
                em.persist(pNuevo);
            }
    ...
    private void chequeoNoUsoDelParametro(ParametroValidacion p) throws UserException {
        Long cant = (Long) em.createQuery(
                ...
                " WHERE vxd.nombreParametro = :nombre " +
                "   AND vxd.validacionPorRecurso.validacion.id = :validacion " +
                "   AND vxd.validacionPorRecurso.recurso.fechaBaja IS NULL " +
                "   AND vxd.fechaDesasociacion IS NULL "
        ).setParameter("nombre", p.getNombre()).setParameter("validacion", p.getValidacion().getId()).getSingleResult();
        if (cant > 0) {
    ...
    /*
     * Crea una ValidacionPorDato, es decir, asocia la validacion al dato. Controla que esta validacion ya no este asociada al dato, es decir que no exista otra
     * ValidacionPorDato viva para el par validacion,dato. Controla que el usuario tenga rol Planificador sobre la agenda asociada Roles permitidos:
     * Planificador
     *
     * @throws UserException
     * @throws ApplicationException
    ...
    private void modificarValidacionesPorDatoDeLaValidacionPorRecurso(ValidacionPorRecurso vxr, List<ValidacionPorDato> validacionesPorDatoModificados) throws UserException, BusinessException {
        ...
        Map<Integer, ValidacionPorDato> mapModificados = new HashMap<>();
    ...
    /*
     * Realiza una baja lógica de la ValidacionPorDato (setea fechaBaja a la fecha actual del sistema). Controla que el usuario tenga rol Planificador sobre la
     * agenda asociada Roles permitidos: Planificador
     *
     * @throws UserException
     */
    @Override
    ...
    private Boolean hayDatosVivos(Validacion validacion) {
        Long cant = (Long) em.createQuery(
                "SELECT count(vxr) FROM ValidacionPorRecurso vxr " + 
                "WHERE vxr.validacion = :validacion " +
                "  AND vxr.recurso.fechaBaja IS NULL " +
                "  AND vxr.fechaBaja is null")
                .setParameter("validacion", validacion)
                .getSingleResult();
        return (cant > 0);
    }
}
