...
@Stateless
public class RecursosBean implements RecursosLocal {
	 ...
	 * null). Se controla que la agenda se encuentre viva. Se permite setear todos
	 * sus atributos menos la fechaBaja. Controla: fechaInicio <> NULL fechaInicio
	 * <= fechaFin o fechaFin == NULL fechaInicioDisp <> NULL fechaInicioDisp <=
	 * fechaFinDisp o fechaFinDisp == NULL fechaInicio <= fechaInicioDisp
	 * ventanaDiasMinimos > 0 ventanaCuposMinimos > 0 cantDiasAGenerar > 0
	 * largoListaEspera > 0 largo del campo Serie menor o igual a 3 cantDiasAGenerar
	 * >= ventanaDiasMinimos Retorna el recurso con su identificador interno.
	 * Controla que el usuario tenga rol Planificador sobre la agenda <b>a</b> Roles
	 * permitidos: Planificador
	 *
	 ...
	 */
	@Override
	public Recurso crearRecurso(Agenda a, Recurso r, String codigoUsuario)
			throws UserException, ApplicationException, BusinessException {
		if (a == null) {
			throw new UserException("debe_haber_una_agenda_seleccionada");
		}
		Agenda aManejada = entityManager.find(Agenda.class, a.getId());
		if (aManejada == null) {
		...
		if (r.getDiasInicioVentanaInternet() == null) {
			throw new UserException("los_dias_de_inicio_de_la_ventana_de_internet_es_obligatorio");
		}
		...
		if (r.getAccionMiPerfil() != null) {
			...
			if (BooleanUtils.isTrue(r.getMiPerfilConHab())) {
			...
			if (BooleanUtils.isTrue(r.getMiPerfilCanHab())) {
			...
			if (BooleanUtils.isTrue(r.getMiPerfilRecHab())) {
	...
	public Recurso crearRecursoImportado(Agenda agenda, Recurso recurso, String codigoUsuario)
			throws UserException, ApplicationException, BusinessException {
		if (agenda == null) {
			throw new UserException("debe_haber_una_agenda_seleccionada");
		}
		Agenda aManejada = entityManager.find(Agenda.class, agenda.getId());
		if (aManejada == null) {
			throw new UserException("no_se_encuentra_la_agenda_especificada");
		}
		// No se puede dar de alta un recurso para una agenda cerrada
		...
		// cantDiasAGenerar >= diasInicioVentanaIntranet + diasVentanaIntranet
		if (recurso.getCantDiasAGenerar()
				.compareTo(recurso.getDiasInicioVentanaIntranet() + recurso.getDiasVentanaIntranet()) < 0) {
			throw new UserException("la_cantidad_de_dias_a_generar_debe_ser_mayor_o_igual_que_la_suma_intranet");
		}
		// cantDiasAGenerar >= diasInicioVentanaInternet + diasVentanaInternet
		if (recurso.getCantDiasAGenerar()
				.compareTo(recurso.getDiasInicioVentanaInternet() + recurso.getDiasVentanaInternet()) < 0) {
			throw new UserException("la_cantidad_de_dias_a_generar_debe_ser_mayor_o_igual_que_la_suma_internet");
		}
		// largoListaEspera > 0
		...
		if (recurso.getReservaMultiplePendienteTiempoMax() != null
				&& recurso.getReservaMultiplePendienteTiempoMax() < 0) {
			throw new UserException("reserva_multiple_pendiente_tiempo_max_debe_ser_mayor_a_cero");
		}
		...
		// Creo una accionMiPerfil por defecto para el recurso importado. Esto es para
		// evitar errores, ya que al importar
		// no se tienen en cuenta las acciones de MiPerfil (si algún dia se agrega lo de
		// acciones MiPerfil al importar/exportar, esto se saca)
	...
	/*
	 * Modifica los datos del recurso <b>r</b>. Controla la unicidad del nombre del
	 * recurso entre todos los recursos vivos (fechaBaja == null). Se permite setear
	 ...
	 * cantDiasAGenerar > 0 largoListaEspera > 0 El largo del campo Serie no puede
	 * ser mayor a 3 cantDiasAGenerar >= ventanaDiasMinimos Si reservaMultiple =
	 * True => se podrá cambiar su valor a reservaMultiple = FALSE solo si no existe
	 * reserva viva con más de una disponibilidad para ese recurso. No pueden quedar
	 * disponibilidades vivas fuera del período fechaInicioDisp y fechaFinDisp. No
	 * pueden quedar reservas vivas fuera del período fechaInicioDisp y
	 * fechaFinDisp. Retorna el recurso con su identificador interno. Controla que
	 * el usuario tenga rol Planificador sobre la agenda <b>a</b> Roles permitidos:
	 * Planificador
	 *
	 * @throws UserException
	 * 
	 ...
	 */
	@Override
	public void modificarRecurso(Recurso recurso, String codigoUsuario)
			throws UserException, BusinessException, ApplicationException {

		Recurso recursoActual = entityManager.find(Recurso.class, recurso.getId());
		...
		// Se setea hora en fecha de inicio disp. 00:00:00
		recurso.setFechaInicioDisp(Utiles.time2InicioDelDia(recurso.getFechaInicioDisp()));
		// Si la fecha de Fin de disponibilidad no es nula, se setea la hora al final
		// del Día.
		if (recurso.getFechaFinDisp() != null) {
			recurso.setFechaFinDisp(Utiles.time2FinDelDia(recurso.getFechaFinDisp()));
		}
		...
		// No pueden quedar disponibilidades vivas fuera del período fechaInicioDisp y
		// fechaFinDisp.
		if (hayDisponibilidadesVivasFueraDelPeriodo(recurso.getId(), recurso.getFechaInicioDisp(),
				recurso.getFechaFinDisp())) {
			throw new UserException(
		...
		if (hayReservasVivasFueraDelPeriodo(recurso.getId(), recurso.getFechaInicioDisp(), recurso.getFechaFinDisp())) {
			throw new UserException(
					"no_se_puede_modificar_las_fechas_porque_quedarian_reservas_fuera_del_periodo_especificado");
		}
		...
		if (recurso.getAccionMiPerfil() != null) {
			...
			// Controlo que haya una sola accion de confirmacion destacada
			if (BooleanUtils.isTrue(recurso.getMiPerfilConHab())) {
			...
			// Controlo que haya una sola accion de cancelacion destacada
			if (BooleanUtils.isTrue(recurso.getMiPerfilCanHab())) {
			...
			// Controlo que haya una sola accion de recordatorio destacada
			if (BooleanUtils.isTrue(recurso.getMiPerfilRecHab())) {
		...
		recursoActual.setTextosRecurso(new HashMap<>());
		...
		if (recurso.getAccionMiPerfil() != null) {
			...
			if (recurso.getAccionMiPerfil().getId() != null) {
				accionActual = entityManager.find(AccionMiPerfil.class, recurso.getAccionMiPerfil().getId());
			}
			if (accionActual == null) {
				accionActual = obtenerAccionMiPerfilPorDefecto(recursoActual);
			}
	...
	/*
	 * Realiza una baja lógica del recurso <b>r</b> (se setea fechaBaja con la fecha
	 * actual del sistema). Controla que no existan disponibilidades vivas
	 * (fechaBaja == null) y futuras (fecha > fecha actual del sistema). Controla
	 * que no existan reservas vivas (estado R o P). Controla que no existan
	 * agrupaciones de datos en el recurso Controla que no existan Validaciones
	 * aplicadas en el recurso Controla que el usuario tenga rol Planificador sobre
	 * la agenda asociada. Roles permitidos: Planificador
	 *
	 * @throws UserException
	 * 
	 * @throws ApplicationException
	...
	@Override
	public void eliminarRecurso(Recurso recurso, TimeZone timezone, String codigoUsuario)
		...
		List<Disponibilidad> disponibilades = entityManager
				.createQuery("SELECT d FROM Disponibilidad d " + "WHERE d.recurso = :recurso "
						+ "  AND d.fecha >= :fecha" + "  AND d.horaFin >= :hora" + "  AND d.fechaBaja is null")
				.setParameter("recurso", recursoActual).setParameter("fecha", new Date())
		...
		List<AgrupacionDato> listaAgrupacion = entityManager
				.createQuery(
						"SELECT ad FROM AgrupacionDato ad " + "WHERE ad.recurso = :recurso AND ad.fechaBaja is null")
		...
		List<ValidacionPorRecurso> listValidacionRecurso = entityManager
	...
	/*
	 * Agrega un DatoDelRecurso <b>d</b> asociándolo al recurso <b>r</b>. Controla
	 * que el usuario tenga rol Planificador sobre la agenda del recurso <b>r</b>
	...
	public DatoDelRecurso agregarDatoDelRecurso(Recurso r, DatoDelRecurso d) throws UserException {
		Recurso recursoActual = entityManager.find(Recurso.class, r.getId());
		if (recursoActual == null) {
			throw new UserException("no_se_encuentra_el_recurso_especificado");
		}
	...
	/*
	 * Modifica los datos del DatoDelRecurso <b>d</b>. Controla que el usuario tenga
	 * rol Planificador sobre la agenda del recurso asociado a <b>d</b> Roles
	 * permitidos: Planificador
	 *
	 * @throws UserException
	 */
	...
	public void modificarDatoDelRecurso(DatoDelRecurso d) throws UserException {
		DatoDelRecurso datoActual = entityManager.find(DatoDelRecurso.class, d.getId());
		if (datoActual == null) {
			throw new UserException("AE10033",
					"No existe la información del recurso que se quiere modificar: " + d.getId().toString());
		}
		...
		if (d.getOrden() == null || d.getOrden() < 0) {
	...
	/*
	 * Realiza una baja física del DatoDelRecurso <b>d</b> Controla que el usuario
	 * tenga rol Planificador sobre la agenda del recurso asociado a <b>d</b>. Roles
	 * permitidos: Planificador
	 *
	 * @throws UserException
	 */
	...
	public List<DatoDelRecurso> consultarDatosDelRecurso(Recurso r) throws ApplicationException, BusinessException {
		...
		try {
			List<DatoDelRecurso> datosDelRecurso = entityManager
					.createQuery("SELECT d from DatoDelRecurso d " + "WHERE d.recurso = :r " + "ORDER BY d.orden")
	...
	public AgrupacionDato agregarAgrupacionDato(Recurso r, AgrupacionDato a)
		...
		} else if (existeAgrupacionPorNombre(a)) {
	 ...
	 * modificar. Controla que el usuario tenga rol Planificador sobre la agenda del
	 * recurso asociado a <b>d</b> Roles permitidos: Planificador
	 *
	 * @throws UserException
	 */
	@Override
	public void modificarAgrupacionDato(AgrupacionDato a) throws UserException, ApplicationException {
		AgrupacionDato agrupacionActual = entityManager.find(AgrupacionDato.class, a.getId());
	...
	/*
	 * Realiza una baja logica de agrupaciones del dato <b>d</b>. Controla que el
	...
	public void eliminarAgrupacionDato(AgrupacionDato a, boolean controlarDatos)
		...
		} else if (agrupacionActual.getDatosASolicitar() != null) {
	...
	public List<AgrupacionDato> consultarAgrupacionesDatos(Recurso r) throws ApplicationException {
		try {
			List<AgrupacionDato> agrupacionDato = entityManager.createQuery("SELECT a from AgrupacionDato a "
					+ "WHERE a.recurso = :r " + "AND a.fechaBaja is null " + "ORDER BY a.orden").setParameter("r", r)
					.getResultList();
	...
	/*
	 * Agrega un DatoASolicitar <b>d</b> asociándolo al recurso <b>r</b>. Controla
	 * que no exista otro dato vivo (fechaBaja == null) con el mismo nombre. Se
	 * permite setear todos sus atributos menos la fechaBaja. Controla que el
	 * usuario tenga rol Planificador sobre la agenda del recurso <b>r</b> Roles
	 * permitidos: Planificador
	 *
	 * @throws UserException
	 * 
	...
	public DatoASolicitar agregarDatoASolicitar(Recurso r, AgrupacionDato a, DatoASolicitar d)
		...
		if (d.getIncluirEnReporte() == true && d.getAnchoDespliegue() <= 0) {
	...
	public DatoASolicitar agregarDatoASolicitarImportar(Recurso recursoActual, AgrupacionDato agrupacionActual,
			DatoASolicitar d) throws UserException, ApplicationException, BusinessException {
		if (recursoActual == null) {
			throw new BusinessException("no_se_encuentra_el_recurso_especificado");
		}
		if (agrupacionActual == null) {
	...
	/*
	 * Modifica los datos de un DatoASolicitar <b>d</b>. Controla que no exista otro
	 * dato vivo (fechaBaja == null) con el mismo nombre. Controla que no existan
	 * datos de reserva asociados al DatoASolicitar. Se permite setear todos sus
	 * atributos menos la fechaBaja. Controla que el usuario tenga rol Planificador
	 * sobre la agenda del recurso <b>r</b> Roles permitidos: Planificador
	 *
	 * @throws UserException
	 * 
	 * @throws ApplicationException
	...
	public void modificarDatoASolicitar(DatoASolicitar d) throws UserException, ApplicationException {
		DatoASolicitar datoActual = entityManager.find(DatoASolicitar.class, d.getId());
		if (datoActual == null) {
			throw new UserException("no_se_encuentra_el_recurso_especificado");
		}
		if (d.getNombre() == null) {
	...
	/*
	 * Realiza una baja lógica del dato a solicitar <b>d</b> (se setea fechaBaja con
	 * la fecha actual del sistema). Controla que el usuario tenga rol Planificador
	 * sobre la agenda asociada. Roles permitidos: Planificador
	 *
	 * @throws UserException
	 */
	@Override
	...
	/*
	 * Agrega un ValorPosible asociándolo al DatoASolicitar. Controla que no exista
	 * otro valor posible con la misma etiqueta y/o valor que se solapen en el
	 * período (fechaDesde, fechaHasta). Controla que el usuario tenga rol
	 * Planificador sobre la agenda del recurso <b>r</b> Roles permitidos:
	 * Planificador
	 *
	 * @throws UserException
	...
	/*
	 * Modifica el ValorPosible. Controla que no exista otro valor posible con la
	 * misma etiqueta y/o valor que se solapen en el período (fechaDesde,
	 * fechaHasta). Permite modificar fecha hasta , ya que significa la vigencia del
	 * valor Controla que el usuario tenga rol Planificador sobre la agenda del
	...
	public void modificarValorPosible(ValorPosible v) throws UserException, ApplicationException {
		ValorPosible valorActual = entityManager.find(ValorPosible.class, v.getId());
		if (valorActual == null) {
		...
		valorActual.setValorEnTraza(v.getValorEnTraza());
		valorActual.setFechaDesde(v.getFechaDesde());
		valorActual.setFechaHasta(v.getFechaHasta());
	}

	/*
	 * Realiza una baja física del ValorPosible Controla que el usuario tenga rol
	...
	public Boolean existeRecursoPorNombre(Recurso r) throws ApplicationException {
		try {
			Long cant = (Long) entityManager
					.createQuery("SELECT count(r) from Recurso r " + "WHERE UPPER(r.nombre) = UPPER(:nombre) "
							+ "AND (r.id <> :id OR :id is null)" + "AND r.agenda = :agenda "
							+ "AND r.fechaBaja IS NULL")
					.setParameter("nombre", r.getNombre().toUpperCase()).setParameter("id", r.getId())
	...
	private Boolean hayReservasVivasFueraDelPeriodo(Integer recursoId, Date desde, Date hasta)
			throws ApplicationException {
		try {
			Long cant = (Long) entityManager
					.createQuery("SELECT count(r) FROM Disponibilidad d JOIN d.reservas r "
							+ "WHERE d.recurso.id = :recursoId "
							+ "  AND (d.fecha < :fecha_desde OR d.fecha > :fecha_hasta )" + "  AND d.presencial=false "
							+ "  AND (r.estado = :reservado OR r.estado = :pendiente) ")
					.setParameter("recursoId", recursoId).setParameter("fecha_desde", desde, TemporalType.DATE)
					.setParameter("fecha_hasta", hasta, TemporalType.DATE).setParameter("reservado", Estado.R)
	...
	private Boolean hayDisponibilidadesVivasFueraDelPeriodo(Integer recursoId, Date desde, Date hasta)
			throws ApplicationException {
		try {
			Long cant = (Long) entityManager
					.createQuery("SELECT count(d) FROM Disponibilidad d " + "WHERE d.recurso.id = :recursoId "
							+ "  AND (d.fecha < :fecha_desde OR d.fecha > :fecha_hasta)" + "  AND d.presencial=false "
							+ "  AND d.fechaBaja IS NULL")
					.setParameter("recursoId", recursoId).setParameter("fecha_desde", desde, TemporalType.DATE)
					.setParameter("fecha_hasta", hasta, TemporalType.DATE).getSingleResult();
			return (cant > 0);
		} catch (Exception e) {
			throw new ApplicationException(e);
		}
	...
	private Boolean existeAgrupacionPorNombre(AgrupacionDato a) throws ApplicationException {
		try {
			Long cant = (Long) entityManager
					.createQuery("SELECT COUNT(a) FROM AgrupacionDato a " + "WHERE a.nombre = :nombre "
							+ "AND (a.id <> :id OR :id is null)" + "AND  a.recurso = :recurso "
							+ "AND  a.fechaBaja IS NULL")
					.setParameter("nombre", a.getNombre()).setParameter("id", a.getId())
	...
	@Override
	public boolean existeDatoASolicPorNombre(String n, Integer idRecurso, Integer idDatoSolicitar)
			throws ApplicationException {
		try {
			Long cant = (Long) entityManager
					.createQuery("SELECT count(d) " + "from DatoASolicitar d " + "WHERE upper(d.nombre) = upper(:n) "
							+ "AND d.recurso.id = :recurso " + "AND d.fechaBaja IS NULL "
							+ "AND (d.id <> :idDatoSolicitar or :idDatoSolicitar is null)")
					.setParameter("n", n).setParameter("recurso", idRecurso)
					.setParameter("idDatoSolicitar", idDatoSolicitar).getSingleResult();
			return (cant > 0);
		} catch (Exception e) {
			throw new ApplicationException(e);
		}
	...
	private Boolean existeDatoASolicPorAgrupacion(Integer a) throws ApplicationException {
		try {
			Long cant = (Long) entityManager.createQuery("SELECT count(d) " + "from DatoASolicitar d "
					+ "WHERE d.agrupacionDato.id = :a " + "AND d.fechaBaja IS NULL").setParameter("a", a)
					.getSingleResult();
			return (cant > 0);
		} catch (Exception e) {
			throw new ApplicationException(e);
	...
	public boolean existeValorPosiblePeriodo(ValorPosible v) throws ApplicationException {
		String queryString = "SELECT count(v) from ValorPosible v "
				...
				+ (v.getId() != null ? " AND v.id <> :id " : "") + "AND  v.dato = :dato " + "AND ("
				+ (v.getFechaHasta() == null
						? " (v.fechaHasta is null) OR (v.fechaHasta is not null and :desde <= v.fechaHasta) "
						: " (v.fechaHasta is null and :hasta >= v.fechaDesde) OR (v.fechaHasta is not null and :hasta >= v.fechaDesde and :desde <= v.fechaHasta) ")
				+ ")";
		try {
			Query query = entityManager.createQuery(queryString);
			query.setParameter("valor", v.getValor()).setParameter("etiqueta", v.getEtiqueta())
					.setParameter("dato", v.getDato()).setParameter("desde", v.getFechaDesde(), TemporalType.TIMESTAMP);
			if (v.getId() != null) {
	 ...
	 * lista de objetos AgrupacionDato ordenada por el atributo orden. La lista de
	 * objetos DatoASolicitar ordenada por (fila,columna)
	 *
	 * @throws BusinessException
	 */
	@SuppressWarnings("unchecked")
	...
	public List<AgrupacionDato> consultarDefinicionDeCampos(Recurso recurso, TimeZone timezone) throws UserException {
		...
		List<AgrupacionDato> agrupacionesDTO = new ArrayList<>();
		List<AgrupacionDato> agrupaciones = entityManager.createQuery(
				"FROM AgrupacionDato a " + "WHERE a.recurso = :r " + "AND a.fechaBaja IS NULL " + "ORDER BY a.orden ")
	...
	/*
	 * Retorna el arbol de AgrupacionDato/DatoASolicitar/ValorPosible. La lista de
	 * objetos AgrupacionDato ordenada por el atributo orden. La lista de objetos
	 * DatoASolicitar ordenada por (fila,columna) Se usa para la consulta de
	 * Reservas por datos a Solicitar y actualmente solo permite traer todos los
	 * valores posibles, no los datos a solicitar ya que esto podría provocar
	 * solapamientos en el formulario
	 *
	 * @throws BusinessException
	 */
	@SuppressWarnings("unchecked")
	...
	public Boolean mostrarDatosASolicitarEnLlamador(Recurso recurso) throws BusinessException {
		...
		Long cantAMostrar = (Long) entityManager
				.createQuery("select count(dato) from DatoASolicitar dato " + "where dato.recurso = :recurso and "
						+ "      dato.fechaBaja is null and " + "      dato.incluirEnLlamador = :incluir ")
				.setParameter("recurso", recurso).setParameter("incluir", true).getSingleResult();
		return cantAMostrar > 0;
	}

	@SuppressWarnings("unchecked")
	private List<DatoASolicitar> obtenerDatosASolicitar(AgrupacionDato agrupacion, TimeZone timezone) {
		List<DatoASolicitar> datosDTO = new ArrayList<>();
		List<DatoASolicitar> datos = entityManager
				.createQuery("FROM DatoASolicitar d " + "WHERE d.agrupacionDato = :agrupacion "
						+ "  AND d.fechaBaja IS NULL " + "ORDER BY d.fila, d.columna ")
				.setParameter("agrupacion", agrupacion).getResultList();
		// Para cada dato a solicitar que sea del tipo Lista obtener los valores
		// posibles.
	...
	/*
	 * Obtiene la lista de Datos a Solicitar para armar la consulta de Reserva por
	 * datos de la reserva. No trae los datos a solicitar dados de baja, debido a
	 * que esto podría provocar solapamientos en el formulario dinámico si recupera
	 * todos los valores posibles de los datos a solicitar de tipo lista aunque ya
	...
	private List<ValorPosible> obtenerValoresPosibles(DatoASolicitar dato, TimeZone timezone) {
		List<ValorPosible> valoresDTO = new ArrayList<>();
		Calendar cal = new GregorianCalendar();
		cal.add(Calendar.MILLISECOND, timezone.getOffset(cal.getTimeInMillis()));
		Date hoy = cal.getTime();
		List<ValorPosible> valores = entityManager
				.createQuery("FROM ValorPosible valor " + "WHERE valor.dato = :dato "
						+ "  AND :hoy >= valor.fechaDesde "
						+ "  AND (valor.fechaHasta IS NULL OR :hoy <= valor.fechaHasta) " + "ORDER BY valor.orden ")
				.setParameter("dato", dato).setParameter("hoy", hoy, TemporalType.DATE).getResultList();
	...
	@Override
	@SuppressWarnings("unchecked")
	public ValorPosible obtenerValorPosiblePorValor(String valor, Integer idDatoSolicitar, TimeZone timezone) {
		Calendar cal = new GregorianCalendar();
		cal.add(Calendar.MILLISECOND, timezone.getOffset(cal.getTimeInMillis()));
		Date hoy = cal.getTime();
	...
	/**
	 * Obtiene la lista de valores posibles para un dato a solicitar de tipo lista
	 * sin filtrar por la vigencia, se usa para la consulta de Reserva por datos de
	 * la reserva.
	 *
	 * @param dato
	 * @return
	...
	private List<ValorPosible> obtenerTodosValoresPosibles(DatoASolicitar dato) {
		...
		List<ValorPosible> valores = entityManager.createQuery(
				"from ValorPosible valor " + "where valor.dato = :d  " + "order by valor.orden, valor.fechaDesde ")
	...
	public List<DatoASolicitar> consultarDatosSolicitar(Recurso recurso) {
		...
		List<DatoASolicitar> datos = entityManager
				.createQuery(
						"FROM DatoASolicitar dato " + "WHERE dato.recurso = :recurso " + "  AND dato.fechaBaja IS NULL "
	...
	private Boolean hayReservasVivas(Recurso recurso, TimeZone timezone) throws ApplicationException {
		try {
			...
			Long cant = (Long) entityManager
					.createQuery("SELECT count(r) FROM Disponibilidad d JOIN d.reservas r "
							+ "WHERE d.recurso = :recurso " + "  AND d.fecha >= :fecha" + "  AND d.horaFin >= :hora"
							+ "  AND r.estado IN ('R','P')")
					.setParameter("recurso", recurso).setParameter("fecha", cal.getTime())
	...
	@Override
	public Recurso copiarRecurso(Recurso recurso, String codigoUsuario)
			throws BusinessException, ApplicationException, UserException {
		// Antes de que haga el entityManager.find(recurso), me guardo la accionMiPerfil
		// que viene en el recurso como transient para no perderla
		...
		// 2
		rCopia.setTextosRecurso(new HashMap<>());
		if (recurso.getTextosRecurso() != null) {
			for (String idioma : recurso.getTextosRecurso().keySet()) {
				TextoRecurso viejo = recurso.getTextosRecurso().get(idioma);
		...
		// 3
		Map<ValidacionPorRecurso, ValidacionPorRecurso> validacionesDelRecurso = new HashMap<>();
	...
	public List<ServicioPorRecurso> consultarServicioAutocompletar(Recurso r) throws BusinessException {
		List<ServicioPorRecurso> servicios = entityManager.createQuery("from ServicioPorRecurso servicio "
				+ "where servicio.recurso = :recurso and " + "      servicio.fechaBaja is null ")
	...
	public Recurso importarRecurso(Agenda a, byte[] b, String versionSAE, String codigoUsuario) throws UserException {
		...
		try {
			...
			recurso.setAgrupacionDatos(new ArrayList<>());
			for (AgrupacionDatoExport agdExp : recursoExp.getAgrupaciones()) {
				...
				agd.setDatosASolicitar(new ArrayList<>());
				for (DatoASolicitarExportar dasExp : agdExp.getDatosAsolicitar()) {
					...
					das.setValoresPosibles(new ArrayList<>());
			...
			if (recursoExp.getTextosRecurso() != null) {
				recurso.setTextosRecurso(new HashMap<>());
	...
	@Override
	public List<RolesUsuarioRecurso> asociarRolesUsuarioRecurso(Integer usuarioId,
			Map<Integer, String[]> rolesRecurso) {
		...
		List<RolesUsuarioRecurso> ret = new ArrayList<>();
	...
	public List<RolesUsuarioRecurso> getRolesUsuarioRecurso(Integer usuarioId) {
		try {
			Query query = entityManager
					.createQuery("SELECT r FROM RolesUsuarioRecurso r WHERE r.id.usuarioId=:usuarioId");
			query = query.setParameter("usuarioId", usuarioId);
			@SuppressWarnings("unchecked")
	...
	public RolesUsuarioRecurso getRolesUsuarioRecurso(Integer usuarioId, Integer recursoId) {
		try {
			Query query = entityManager.createQuery(
					"SELECT r FROM RolesUsuarioRecurso r WHERE r.id.usuarioId=:usuarioId AND r.id.recursoId=:recursoId");
			RolesUsuarioRecurso ret = (RolesUsuarioRecurso) query.setParameter("usuarioId", usuarioId)
					.setParameter("recursoId", recursoId).getSingleResult();
			return ret;
		} catch (Exception ex) {
			return null;
		}
	...
	public AccionMiPerfil obtenerAccionMiPerfilDeRecurso(Integer recursoId) {
		try {
			Query query = entityManager
					.createQuery("SELECT a FROM AccionMiPerfil a WHERE a.recurso.id=:recursoId order by a.id desc");
			query.setMaxResults(1);
			return (AccionMiPerfil) query.setParameter("recursoId", recursoId).getSingleResult();
		} catch (Exception ex) {
			return null;
		}
	}
	...
	public String cargaMasiva(Agenda a, byte[] b, String codigoUsuario, TimeZone timezone, Integer organismoActual)
			throws UserException {
		...
		try {
			...
			// boolean lockOk = (boolean)entityManager.createNativeQuery("SELECT
			// pg_advisory_unlock("+LOCK_ID+")").getSingleResult();
			// Intentar obtener el lock
			boolean lockOk = (boolean) entityManager
					.createNativeQuery("SELECT pg_try_advisory_xact_lock(" + LOCK_ID + ")").getSingleResult();
			if (!lockOk) {
			...
			String[] memberFieldsToBindTo = { "nombreRecurso", "Direccion", "Latitud", "Longitud", "Telefonos",
					"Horarios", "fechaInicio", "fechaFin", "Lunes", "Martes", "Miercoles", "Jueves", "Viernes",
					"Sabado", "Domingo" };
			strategy.setColumnMapping(memberFieldsToBindTo);

			CsvToBean<CamposCSV> csvToBean = new CsvToBeanBuilder(reader).withMappingStrategy(strategy).withSkipLines(1)
			...
			while (cargaMasivaIterator.hasNext()) {
				...
				if (!recursosImportados.isEmpty()) {
					for (Recurso r : recursosImportados) {
						if (r.getNombre().equals(cargaMasivaLinea.getNombreRecurso())) {
							// logger.error("El recurso duplicado es.. " +
							// cargaMasivaLinea.getNombreRecurso());
				...
				if (!recursoDuplicado) {
					try {
						Recurso recurso = this.guardarRecurso(cargaMasivaLinea, a, codigoUsuario, organismoActual);
						if (recurso != null) {
							...
							try {
								...
								if (errorLinea && recursoEsNuevo) {
									...
									throw new UserException("Error creando las disponibilidades  del recurso "
											+ cargaMasivaLinea.getNombreRecurso());
								} else if (errorLinea) {
	...
	private Recurso guardarRecurso(CamposCSV cargaMasivaLinea, Agenda a, String codigoUsuario, Integer organismoActual)
			throws UserException {
		...
		try {
			diasRecursosNuevos = Integer
					.valueOf(confBean.getString("CARGA_MASIVA_DIAS_RECURSOS_NUEVOS", organismoActual));
		} catch (Exception nfEx) {
			// logger.error("Error al obtener las configuraciones para los días de los
			// recursos");
		}

		try {
			diasRecursosExistentes = Integer
					.valueOf(confBean.getString("CARGA_MASIVA_DIAS_RECURSOS_EXISTENTES", organismoActual));
		} catch (Exception nfEx) {
			// logger.error("Error al obtener las configuraciones para los días de los
			// recursos");
		}
		...
		if (!recs.isEmpty()) {
			try {
				recurso = recs.get(0);
				// logger.info("Ya existe un recurso llamado "+recurso.getNombre()+" en la
				// agenda "+a.getNombre());
				Boolean fechaDentroRango = Boolean.FALSE;
				// comparar que las fechas esten dentro de las fechas de atención al cliente
				// la fecha inicio esta despues de la fecha inicio disp y antes de la fecha fin
				// disp
				if (fechaInicio.compareTo(recurso.getFechaInicioDisp()) == 0
						|| fechaInicio.after(recurso.getFechaInicioDisp())) {
					fechaDentroRango = Boolean.TRUE;
				}

				if (fechaInicio.compareTo(recurso.getFechaFinDisp()) == 0
						|| fechaInicio.before(recurso.getFechaFinDisp())) {
					fechaDentroRango = Boolean.TRUE;
				} else {
				...
				if (!StringUtils.isBlank(cargaMasivaLinea.getLatitud())
						&& !StringUtils.isBlank(cargaMasivaLinea.getLongitud())) {
			...
			if (recursoEsNuevo && fechaInicio != null && fechaFin != null) {
				...
				for (Date date = start.getTime(); start.before(end); start.add(Calendar.DATE,
					...
					if (crearCupos) {
						...
						for (Date turnos = inicioTurnos.getTime(); inicioTurnos.before(finTurnos); inicioTurnos
								.add(Calendar.MINUTE, duracion), turnos = inicioTurnos.getTime()) {
				...
				for (Date date = start.getTime(); start.before(end); start.add(Calendar.DATE,
					...
					if (result && crearCupos) {
						...
						// Se pregunta si existen disponibilidades para una fecha en especifico con el
						// mismo rango de horas
						List<Disponibilidad> disponibilidades = disponibilidadEJB
								.obtenerDisponibilidadesRangoHoraInicio(date, dispInicio, dispFin, recurso);

						if (!disponibilidades.isEmpty()) {

							Disponibilidad dispIni = disponibilidadEJB.obtenerDisponibilidadEnHoraInicioMin(recurso,
									date);
							Disponibilidad dispF = disponibilidadEJB.obtenerDisponibilidadEnHoraInicioMax(recurso,
									date);
							...
							if (dispInicio.compareTo(dispIni.getHoraInicio()) == 0
									&& horaFin.getTime().compareTo(dispF.getHoraFin()) == 0) {
								// las horas son iguales
								// logger.info("las horas son iguales actualiza las disponibilidades en los
								// cupos");
