...
@Stateless
public class LlamadasBean implements LlamadasLocal {
    ...
    /*
     * Obtiene la siguiente reserva del dia en la lista de espera y automáticamente le cambia el estado a U (Utilizada) y genera una llamada con la reserva y el
     * puesto. Utiliza el campo VERSION en la entidad Rerserva para realizar un bloque optimista Si dos transacciones toman la misma reserva para cambiarle el
     * estado, solo uno tendra exito, por lo que este metodo, frente a una excepcion de bloqueo optimista vuelve a intenar con la siguiente reserva hasta
     * obtener una o que se acaben las reservas en la lista de espera, en cuyo caso retorna null.
     *
     * @throws BusinessException
     */
    @SuppressWarnings("unchecked")
    @Override
    public Reserva llamarASiguientePorReservaId(Recurso recurso, Integer puesto, boolean presencial, Integer idReserva) throws UserException {
        if (recurso == null) {
        ...
        if (idReserva > 0 && idReserva != null) {
            query = em.createQuery("SELECT r.id "
            + "FROM Reserva r "
            + "JOIN r.disponibilidades d "
            + "WHERE d.recurso.id = :recurso " 
            + "  AND " + (presencial ? "d.presencial=true" : "d.presencial=false") + " "
            ...
            .setParameter("estado", Estado.R)
            .setParameter("hoy", new Date(), TemporalType.DATE)
            .setParameter("reservaId", idReserva);
        }
    ...
    /*
     * Vuelve a colocar la reserva indicada para ser llamada, es decir, simula el comportamiento del metodo siguienteEnEspera para una reserva que ya fue
     * utilizada, generando una llamada con la reserva y el puesto.
     *
     * @throws BusinessException
     */
    @Override
    ...
    @SuppressWarnings("unchecked")
    @Override
    public List<Llamada> obtenerLlamadasExterno(List<Recurso> recursos, Integer cantLlamadas) throws UserException {
        ...
        List<Object[]> llamadas = (List<Object[]>) query.getResultList();
        List<Llamada> llamadasDTO = new ArrayList<>();
        Map<Integer, Integer> puestos = new HashMap<>();
        for (Object[] llamada : llamadas) {
            ...
            if (puestos.containsKey((Integer)llamada[5])) {
                dto.setPuesto(null);
            } else {
    ...
    @SuppressWarnings("unchecked")
    @Override
    public List<Llamada> obtenerLlamadasInterno(List<Recurso> recursos, Integer cantLlamadas) throws UserException {
        if (recursos == null || recursos.isEmpty()) {
            throw new UserException("debe_especificar_el_recurso");
        }

        Query query = em.createQuery("SELECT r "
                + "FROM Reserva r, IN(r.disponibilidades) d "
                + "WHERE d.recurso.id IN (:recursos) "
                + "  AND d.fecha = :hoy "
                + "  AND d.presencial = true"
                + "  AND r.estado=:reservado"
                + "  AND NOT EXISTS(SELECT 1 FROM Llamada l WHERE l.reserva.id=r.id)"
                + "ORDER BY r.id ASC");
        List<Integer> recursosIds = new ArrayList<>();
        for (Recurso recurso : recursos) {
            recursosIds.add(recurso.getId());
        ...
        query.setParameter("recursos", recursosIds);
        query.setParameter("hoy", new Date(), TemporalType.DATE);
        query.setParameter("reservado", Estado.R);
        query.setMaxResults(cantLlamadas);
        ...
        for (Reserva reserva : reservas) {
            ...
            dto.setEtiqueta(reserva.getDocumento());
            dto.setFecha(reserva.getFechaHora());
            dto.setHora(reserva.getFechaHora());
            dto.setNumero(reserva.getNumero());
            dto.setPuesto(null);
            dto.setReserva(reserva);
            dto.setRecurso(reserva.getDisponibilidades().get(0).getRecurso());
            llamadasDTO.add(dto);
        }
        return llamadasDTO;
    }
