...
@Stateless
public class AgendarReservasBean implements AgendarReservasLocal {
	...
	/*
	 * Retorna la agenda cuyo nombre sea <b>id</b> siempre y cuando esta viva
	 * (fechaBaja == null).
	 *
	...
	public Agenda consultarAgendaPorId(Integer id) throws ApplicationException, BusinessException {
		...
		try {
			return (Agenda) entityManager.createQuery("select a from Agenda a where a.id=:id and a.fechaBaja is null")
					.setParameter("id", id).getSingleResult();
		} catch (NonUniqueResultException | NoResultException nurEx) {
			throw new ApplicationException("no_se_encuentra_la_agenda_especificada");
		}
	}

	/*
	 * Retorna el recurso cuyo que esté asociada a la reserva con el identificador
	 * <b>reservId</b>
	 *
	 * @throws ApplicationException
	 * 
	...
	public Recurso consultarRecursoPorReservaId(Integer reservId) throws ApplicationException, BusinessException {
		...
		try {
			return (Recurso) entityManager.createQuery(
					"select distinct d.recurso from Reserva r join r.disponibilidades d where r.id=:reservaId and r.estado ='R'")
					.setParameter("reservaId", reservId).getSingleResult();
	...
	/*
	 * Retorna el recurso cuyo id sea <b>id</b> siempre y cuando esta vivo
	 * (fechaBaja == null). Controla que el usuario tenga rol FuncionarioAtencion
	 * sobre la agenda <b>a</b> Roles permitidos: FuncionarioAtencion
	 *
	 * @throws BusinessException
	 * 
	 * @throws ApplicationException
	...
	public Recurso consultarRecursoPorId(Agenda a, Integer id) throws ApplicationException, BusinessException {
		...
		try {
			return (Recurso) entityManager
					.createQuery("from Recurso r where r.id = :id and r.fechaBaja is null and r.agenda = :a")
					.setParameter("id", id).setParameter("a", a).getSingleResult();
	 ...
	 * Solo retorna las agendas para las que el usuario tenga rol
	 * FuncionarioAtencion. Solo retorna las agendas tales que una posterior llamada
	 * a consultarRecursos retorne por lo menos un recurso. Roles permitidos:
	 * FuncionarioAtencion
	 *
	 * @throws ApplicationException
	 * 
	...
	public List<Agenda> consultarAgendas() throws ApplicationException, BusinessException {
		List<Agenda> agendas;
		try {
			agendas = entityManager.createQuery("from Agenda a where a.fechaBaja is null order by a.nombre")
					.getResultList();
		} catch (Exception e) {
			throw new ApplicationException(e);
		}
		List<Agenda> agendasValidas = new ArrayList<>();
		for (Agenda agenda : agendas) {
			if (!consultarRecursos(agenda).isEmpty()) {
	 ...
	 * Solo retorna aquellos recursos tales que la fecha actual este comprendida
	 * entre la fechaInicio y fechaFin del recurso. Controla que el usuario tenga
	 * rol FuncionarioAtencion sobre la agenda <b>a</b> Roles permitidos:
	 * FuncionarioAtencion
	 *
	 * @throws ApplicationException
	 * 
	...
	public List<Recurso> consultarRecursos(Agenda a) throws ApplicationException, BusinessException {
		...
		try {
			return entityManager
					.createQuery("from Recurso r where " + "r.agenda = :a and " + "r.fechaBaja is null and  "
							+ "r.fechaInicio <= :ahora and " + "(r.fechaFin is null or r.fechaFin >= :ahora)"
							+ "order by r.descripcion")
					.setParameter("a", a).setParameter("ahora", new Date(), TemporalType.TIMESTAMP).getResultList();
		} catch (Exception e) {
			throw new ApplicationException(e);
		}
	}
	...
	@Override
	public void cancelarReserva(Integer idEmpresa, Integer idAgenda, Integer idRecurso, Integer idReserva)
	...
	private void cancelarReserva(Empresa empresa, Recurso recurso, Reserva reserva, boolean masiva)
			throws UserException {
		if (recurso == null) {
			throw new UserException("debe_especificar_el_recurso");
		}
		...
		// Determinar si aún se está a tiempo de cancelar la reserva (no aplica si es
		// masiva)
		if (!masiva) {
		...
		reservaDTO.setUcancela(ctx.getCallerPrincipal().getName().toLowerCase()); // el usuario de sesion es el que esta
																					// cancelando la reserva anterior y
																					// ese dato lo enviamos hacia GAP
		...
		// Determinar la fecha de liberación según la cancelación sea inmediata o
		// diferida
		if (recurso.getCancelacionTipo() == null || FormaCancelacion.I.equals(recurso.getCancelacionTipo())) {
			...
			// Calcular la fecha y hora de liberación como la hora actual más el
			// desplazamiento
		...
		if (transaccionId != null) {
			trazaBean.registrarLinea(empresa, reserva, transaccionId, recurso.getNombre(),
					ServiciosTrazabilidadBean.Paso.CANCELACION);
		}
		// Publicar la novedad
	...
	/*
	 * Consulta una reserva por numero y para un recurso. En caso contrario
	 * simplemente se busca la reserva por id y se la retorna (sin validar nada)
	 */
	@Override
	 ...
	 */
	@Override
	public Reserva marcarReserva(Disponibilidad disponibilidad, OrigenReserva origen, TokenReserva token,
			String ipOrigen) throws UserException {
		...
		Reserva reserva = helper.crearReservaPendiente(disponibilidad, origen, token, ipOrigen);
		// Chequeo que el cupo real no de negativo
		// Si el cupo real da negativo, elimino la reserva pendiente y cancelo la
		// operacion
	...
	@Override
	public Reserva marcarReservaValidandoDatos(Disponibilidad disponibilidad, OrigenReserva origen, Reserva reserva,
			TokenReserva token, String ipOrigen) throws UserException {
		if (disponibilidad == null) {
			throw new UserException("debe_especificar_la_disponibilidad");
		}
		disponibilidad = entityManager.find(Disponibilidad.class, disponibilidad.getId());
		...
		Reserva reservaNueva = helper.crearReservaPendiente(disponibilidad, origen, token, ipOrigen);
		List<Reserva> reservasPrevias = helper.validarDatosReservaPorClave(recurso, disponibilidad, campos, valores,
				reserva.getTramiteCodigo(), null);
		// Si hay reservas repetidas y no son la misma que la pasada por parámetro se
		// lanza una excepcion.
		if (!reservasPrevias.isEmpty()) {
			for (Reserva reservaPrevia : reservasPrevias) {
				if (!reservaPrevia.getId().equals(reserva.getId())) {
					...
					List<String> nombreCamposClave = new ArrayList<>();
		...
		// Validaciones por IP (si el recurso lo exige y la reserva incluye la IP; no
		// incluye la IP si es desde el backend o autogestión)
		...
		// Si el cupo real da negativo, se elimina la reserva pendiente y cancela la
		// operación
	...
	public void validarDatosReserva(Empresa empresa, Reserva reserva)
		...
		// Armo las estructuras de Map necesarias para poder ejecutar las validaciones
		// sobre los datos de la reserva
		...
		List<Reserva> reservasPrevias = helper.validarDatosReservaPorClave(recurso, disponibilidad, campos, valores,
				reserva.getTramiteCodigo(), reserva.getId());
		if (!reservasPrevias.isEmpty()) {
			List<String> nombreCamposClave = new ArrayList<>();
			for (DatoASolicitar campo : campos) {
				if (campo.getEsClave()) {
	...
	/*
	 * Confirma la reserva. Solo aplica si la reserva existe y está en el estado P
	 * (Pendiente). Si la reserva debe tener datos, estos son exigidos y validados
	 * en este metodo, incluyendo la verificacion de clave unica Si se trata de una
	 * reserva individual (el token es nulo) queda en estado Reservada (confirmada);
	 * si se trata de una reserva perteneciente a un bloque de reservas múltiples
	 * permanece en estado pendiente
	 *
	 * @throws ApplicationException
	 * 
	 * @throws BusinessException
	 * 
	 * @return Reserva: Devuelve la reserva pues se le ha asignado un numero unico
	 * de reserva dentro de la hora a la que pertenece.
	 * 
	 * @throws UserException
	 * 
	...
	public Reserva confirmarReserva(Empresa empresa, Reserva reserva, String transaccionPadreId, Long pasoPadre,
			boolean inicioAsistido, InformacionRequestDTO informacionRequestDTO) throws ApplicationException,
			BusinessException, ValidacionException, AccesoMultipleException, UserException {
		if (reserva == null || reserva.getDatosReserva() == null) {
			throw new BusinessException("debe_especificar_la_reserva");
		}
		// Esto se lee antes de recuperar la reserva de la base porque puede haber
		// cambiado
		Set<DatoReserva> datosNuevos = reserva.getDatosReserva();
		...
		if (tramiteNombre == null && tramiteCodigo != null) {
			TramiteAgenda tramite = consultarTramitePorCodigo(
					reserva.getDisponibilidades().get(0).getRecurso().getAgenda(), tramiteCodigo);
			if (tramite != null) {
		...
		// Validar los datos de la reserva: campos obligatorios, campos clave
		// duplicados, validaciones extendidas
		...
		// Confirmo la reserva, paso el estado a Reservada (si no es múltiple) y le
		// asigno el numero de reserva dentro de la disponibilidad.
		...
		// String origen = (inicioAsistido?"C":"W");
		...
		// Si no es parte de una reserva múltiple se registra en el sistema de
		// trazabilidad y en el de novedades
		if (reserva.getToken() == null) {
			// Registrar en el sistema de trazas
			String transaccionId = trazaBean.armarTransaccionId(empresa.getOid(), reserva.getTramiteCodigo(),
					reserva.getId());
			if (empresa.getOid() != null && transaccionId != null) {
				// Registrar el cabezal en el sistema de trazabilidad del PEU
				String trazaGuid = trazaBean.registrarCabezal(empresa, reserva, transaccionId,
						reserva.getTramiteCodigo(), inicioAsistido, transaccionPadreId, pasoPadre,
						informacionRequestDTO);
				if (trazaGuid != null) {
				...
				// ToDo: esto habria que hacerlo solo si pudo invocar el cabezal; en otro caso
				// solo habría que guardar la invocación
				// en la base de datos para futuros intentos
				trazaBean.registrarLinea(empresa, reserva, transaccionId, recurso.getNombre(),
						ServiciosTrazabilidadBean.Paso.RESERVA);
				// Publicar la novedad
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<Disponibilidad> obtenerDisponibilidades(Recurso recurso, VentanaDeTiempo ventana, TimeZone timezone)
			throws UserException {
	...
	@SuppressWarnings("unchecked")
	public List<Disponibilidad> obtenerDisponibilidades(Recurso recurso, VentanaDeTiempo ventana, TimeZone timezone,
			boolean ajustarVentanaSegunAhora) throws UserException {
		if (recurso == null) {
			throw new UserException("debe_especificar_el_recurso");
		}
		if (ventana == null) {
		...
		Date ahora = TimeZoneUtils.applyTimeZone(timezone, new GregorianCalendar());
		...
		String eql = "SELECT d " + "FROM Disponibilidad d " + "WHERE d.recurso IS NOT NULL "
				+ "  AND d.presencial = false " + "  AND d.recurso = :r " + "  AND d.fechaBaja IS NULL "
				+ "  AND d.horaInicio >= :fi " + "  AND d.horaFin <= :ff " + "ORDER BY d.fecha ASC, d.horaInicio ";
		List<Disponibilidad> disponibilidades = entityManager.createQuery(eql).setParameter("r", recurso)
				.setParameter("fi", ventana.getFechaInicial(), TemporalType.TIMESTAMP)
				.setParameter("ff", ventana.getFechaFinal(), TemporalType.TIMESTAMP).getResultList();
		// Determinar las reservas vivas
		// No se debe considerar las disponibilidades presenciales
		// Para las reservas canceladas hay que ver si ya se liberó el cupo o aún no
		String cons = "SELECT d.id, d.fecha, d.horaInicio, COUNT(r) " + "FROM Disponibilidad d " + "JOIN d.reservas r "
				+ "WHERE d.recurso IS NOT NULL " + "  AND d.recurso = :recurso " + "  AND d.fechaBaja IS NULL "
				+ "  AND d.horaInicio >= :finicio " + "  AND d.horaFin <= :ffin "
				+ "  AND (d.fecha <> :finicio OR d.horaInicio >= :fiCompleta) "
				+ "  AND (r.estado <> :cancelado OR r.fechaLiberacion>=:ahora) "
				+ "GROUP BY d.id, d.fecha, d.horaInicio " + "ORDER BY d.fecha asc, d.horaInicio ASC ";
		List<Object[]> cantReservasVivas = entityManager.createQuery(cons).setParameter("recurso", recurso)
				...
				.setParameter("cancelado", Estado.C).setParameter("ahora", new Date(), TemporalType.TIMESTAMP)
				.getResultList();
	 ...
	 * cumpliendo con los periodos para realizar reservas indicado en el recurso, es
	 * decir: El inicio del calendario es el primer dia que tenga asigando
	 * disponibilidades, que este dentro del periodo de vigencia de las
	 * disponibilidades indicado en el recurso y sea mayor o igual al dia actual. El
	 * fin del calendario es el ultimo dia que tenga asignado disponibilidades, que
	 * este dentro del periodo de vigencia de las disponibilidades indicado en el
	 * recurso y tratando de satisfacer que: La diferencia entre fin - inicio sea
	 * mayor o igual a ventanaDiasMinimos. Los cupos disponibles en la ventana sea
	 * mayor o igual a ventanaCuposMinimos. Si esto no fuera posible se debe
	 * agrandar la ventana hasta llegar a la ultima disponibilidad sin pasarse del
	 * periodo de fin de disponibilidades.
	 *
	 * En este caso la ventana de tiempo retornada representa dias, por lo cual, la
	 * hora del dia esta asignada a las 00:00:00.
	 *
	 * @param recurso
	 * 
	 * @return
	...
	/*
	 * Devuelve una lista de enteros indicando los cupos disponibles, por dia,
	 * asignados al recurso. Es decir, para cada dia de la ventana de tiempo,
	 * devuelve la cantidad de cupos disponibles en ese dia para el recurso dado.
	 *
	 * Si el dia de inicio de la ventana es "hoy", solo se toman en cuenta los cupos
	 * a partir de "ahora", es decir, las disponibilidades cuya horaInicio sea mayor
	 * al tiempo actual. Para los dias de la ventana que no tengan disponibilidad o
	 * sean dias del pasado ( < "hoy"), se devolvera un valor negativo indicando que
	 * ese dia es no-agendable. Para los dias de la ventana que no les queden cupos
	 * libres, se devolvera 0. @param recurso @param ventana @return
	 */
	@Override
	public List<Integer> obtenerCuposPorDia(Recurso recurso, VentanaDeTiempo ventana, TimeZone timezone)
			throws UserException {
		if (recurso == null) {
			throw new UserException("debe_especificar_el_recurso");
		}
		...
		// Armar la lista de resultados indicando los cupos para todos los dias
		// solicitados en la ventana
		List<Integer> cuposXdia = helper.obtenerCuposXDia(ventana, cuposAsignados, cuposConsumidos);
		return cuposXdia;
	}

	/*
	 * Completa campos de la reserva. Los datos necesarios para el servicio son
	 ...
	 * 
	 * @return Map<String, Object>: devuelve un Map con los valores que se desea
	 * asignar a cada campo.
	 */
	@Override
	...
	/*
	 * public void enviarComunicacionesConfirmacion(Empresa empresa, String
	 * linkCancelacion, String linkModificacion, Reserva reserva, String idioma,
	 * String formatoFecha, String formatoHora) throws UserException { //Enviar mail
	 * String cuerpo = enviarMailConfirmacion(linkCancelacion, linkModificacion,
	 * reserva, idioma, formatoFecha, formatoHora); boolean mailOk = (cuerpo!=null);
	 * //Almacenar datos para SMS y TextoAVoz, si corresponde
	 * almacenarSmsYTav(Comunicacion.Tipo2.RESERVA, reserva, formatoFecha,
	 * formatoHora); //Enviar aviso a MiPerfil boolean mipOk =
	 * enviarAvisoMiPerfil(empresa, reserva, Comunicacion.Tipo2.RESERVA); if(!mailOk
	 * && !mipOk) { throw new UserException(
	 * "no_se_pudo_enviar_notificacion_de_confirmacion_tome_nota_de_los_datos_de_la_reserva"
	 * ); } }
	 */
	/*
	 * Este método es similares a enviarComunicacionesConfirmacion pero aplica a un
	 * TokenReserva que contenga múltiples reservas en lugar de a una sola.
	 */
	/*
	 * public void enviarComunicacionesConfirmacion(Empresa empresa, String
	 * templateLinkCancelacion, String templateLinkModificacion, TokenReserva
	 * tokenReserva, String idioma, String formatoFecha, String formatoHora) throws
	 * UserException { List<Reserva> reservas = obtenerReservasMultiples
	 * (tokenReserva.getId(), false); StringBuilder cuerpos = new StringBuilder();
	 * //Para cada reserva enviar la comunicación y añadir el cuerpo al texto que se
	 * enviará a la persona que realiza las reservas for(Reserva reserva : reservas)
	 * { if(reserva.getEstado().equals(Estado.R)) { //Enviar mail String
	 * linkCancelacion = templateLinkCancelacion.replace("{idReserva}",
	 * reserva.getId().toString()); String linkModificacion =
	 * templateLinkModificacion.replace("{idReserva}", reserva.getId().toString());
	 * String cuerpo; try { cuerpo = enviarMailConfirmacion(linkCancelacion,
	 * linkModificacion, reserva, idioma, formatoFecha, formatoHora);
	 * }catch(Exception ex) { cuerpo = null; } if(cuerpo == null) { cuerpo =
	 * "Reserva id {idReserva} confirmada.".replace("{idReserva}",
	 * reserva.getId().toString()); }
	 * cuerpos.append("<br />Documento: "+reserva.getTipoDocumento()+" "+reserva.
	 * getNumeroDocumento()+"<br /><br />" + cuerpo);
	 * cuerpos.append("<br />*************************<br />"); //Almacenar datos
	 * para SMS y TextoAVoz, si corresponde
	 * almacenarSmsYTav(Comunicacion.Tipo2.RESERVA, reserva, formatoFecha,
	 * formatoHora); //Enviar aviso a MiPerfil enviarAvisoMiPerfil(empresa, reserva,
	 * Comunicacion.Tipo2.RESERVA); } } //Enviar las comunicaciones a la persona que
	 * realiza las reservas if(tokenReserva.getCorreoe()!=null) { try { String
	 * cuerpo = cuerpos.toString(); MailUtiles.enviarMail(tokenReserva.getCorreoe(),
	 * "Confirmación de reserva múltiple", cuerpo, MailUtiles.CONTENT_TYPE_HTML);
	 * //No se guarda el cuerpo porque podría ser muy grande si hay muchas reservas,
	 * y puede verse en el envío de cada reserva individual Comunicacion
	 * comunicacion = new Comunicacion(Tipo1.EMAIL, Tipo2.RESERVAMULTIPLE,
	 * tokenReserva.getCorreoe(), tokenReserva.getRecurso(), tokenReserva, "");
	 * comunicacion.setProcesado(true); entityManager.persist(comunicacion);
	 * }catch(MessagingException mEx) { try { Comunicacion comunicacion = new
	 * Comunicacion(Tipo1.EMAIL, Tipo2.RESERVAMULTIPLE, tokenReserva.getCorreoe(),
	 * tokenReserva.getRecurso(), tokenReserva, "");
	 * entityManager.persist(comunicacion); }catch(Exception ex) { logger.
	 * error("No se pudo enviar una comunicación por confirmación de reserva múltiple"
	 * , ex); ex.printStackTrace(); } } } }
	 */
	/*
	 * public void enviarComunicacionesCancelacion(Empresa empresa, Reserva reserva,
	 * String idioma, String formatoFecha, String formatoHora) throws UserException
	 * { enviarComunicacionesCancelacion(empresa, reserva, idioma, formatoFecha,
	 * formatoHora, null, null); }
	 */
	/*
	 * private void enviarComunicacionesCancelacion(Empresa empresa, Reserva
	 * reserva, String idioma, String formatoFecha, String formatoHora, String
	 * asunto, String cuerpo) throws UserException { //Enviar mail boolean mailOk =
	 * enviarMailCancelacion(reserva, idioma, formatoFecha, formatoHora, asunto,
	 * cuerpo); //Almacenar datos para SMS y TextoAVoz, si corresponde
	 * almacenarSmsYTav(Comunicacion.Tipo2.CANCELA, reserva, formatoFecha,
	 * formatoHora); //Enviar aviso a MiPerfil boolean mipOk =
	 * enviarAvisoMiPerfil(empresa, reserva, Comunicacion.Tipo2.CANCELA); //Si no se
	 * pudo enviar mail ni aviso a mi perfil lanzar una excepción if(!mailOk &&
	 * !mipOk) { throw new
	 * UserException("no_se_pudo_enviar_notificacion_de_cancelacion"); } }
	 */
	/*
	 * private String enviarMailConfirmacion(String linkCancelacion, String
	 * linkModificacion, Reserva reserva, String idioma, String formatoFecha, String
	 * formatoHora) { //Se envía el mail obligatorio al usuario Recurso recurso =
	 * null; String email = null; try { recurso =
	 * reserva.getDisponibilidades().get(0).getRecurso(); Agenda agenda =
	 * recurso.getAgenda(); TextoAgenda textoAgenda = null;
	 * if(agenda.getTextosAgenda()!=null) { textoAgenda =
	 * agenda.getTextosAgenda().get(idioma); } if(textoAgenda == null) {
	 * for(TextoAgenda ta : agenda.getTextosAgenda().values()) {
	 * if(ta.isPorDefecto()) { textoAgenda = ta; } } } if(textoAgenda == null) {
	 * textoAgenda = new TextoAgenda(); } //Obtener el mail del usuario, que es el
	 * dato a solicitar llamado "Mail" en la agrupacion que no se puede borrar
	 * String cuerpo = null; cuerpo = textoAgenda.getTextoCorreoConf(); if(cuerpo !=
	 * null && !cuerpo.isEmpty()) { cuerpo =
	 * Metavariables.remplazarMetavariables(cuerpo, reserva, formatoFecha,
	 * formatoHora, linkCancelacion, linkModificacion); for(DatoReserva dato :
	 * reserva.getDatosReserva()) { DatoASolicitar datoSol =
	 * dato.getDatoASolicitar();
	 * if(DatoASolicitar.CORREO_ELECTRONICO.equalsIgnoreCase(datoSol.getNombre()) &&
	 * !datoSol.getAgrupacionDato().getBorrarFlag()) { email = dato.getValor();
	 * MailUtiles.enviarMail(email, "Confirmación de reserva", cuerpo,
	 * MailUtiles.CONTENT_TYPE_HTML); Comunicacion comunicacion = new
	 * Comunicacion(Tipo1.EMAIL, Tipo2.RESERVA, email, recurso, reserva, cuerpo);
	 * comunicacion.setProcesado(true); entityManager.persist(comunicacion); } }
	 * return cuerpo; }else { //Indicar que no se pudo enviar el mail return null; }
	 * }catch(MessagingException mEx) {
	 * logger.warn("No se pudo enviar el mail de confirmación", mEx); try {
	 * if(recurso != null) { if(email == null) { email = "***"; } Comunicacion
	 * comunicacion = new Comunicacion(Tipo1.EMAIL, Tipo2.RESERVA, email, recurso,
	 * reserva, null); entityManager.persist(comunicacion); } }catch(Exception ex) {
	 * // } //Indicar que no se pudo enviar el mail return null; } }
	 */
	/*
	 * private boolean enviarMailCancelacion(Reserva reserva, String idioma, String
	 * formatoFecha, String formatoHora, String asunto, String cuerpo) { //Se envía
	 * el mail obligatorio al usuario Recurso recurso = null; try { recurso =
	 * reserva.getDisponibilidades().get(0).getRecurso(); if(cuerpo == null) {
	 * Agenda agenda = recurso.getAgenda(); TextoAgenda textoAgenda = null;
	 * if(agenda.getTextosAgenda()!=null) { textoAgenda =
	 * agenda.getTextosAgenda().get(idioma); } if(textoAgenda == null) {
	 * for(TextoAgenda ta : agenda.getTextosAgenda().values()) {
	 * if(ta.isPorDefecto()) { textoAgenda = ta; } } } if(textoAgenda != null) {
	 * cuerpo = textoAgenda.getTextoCorreoCanc(); } } if(asunto == null) { asunto =
	 * "Cancelación de reserva"; } //Obtener el mail del usuario, que es el dato a
	 * solicitar llamado "Mail" en la agrupacion que no se puede borrar String
	 * emailTo = null; for(DatoReserva dato : reserva.getDatosReserva()) {
	 * DatoASolicitar datoSol = dato.getDatoASolicitar();
	 * if(DatoASolicitar.CORREO_ELECTRONICO.equalsIgnoreCase(datoSol.getNombre()) &&
	 * !datoSol.getAgrupacionDato().getBorrarFlag()) { emailTo = dato.getValor(); }
	 * } if(emailTo != null) { if(cuerpo != null && !cuerpo.isEmpty()) { cuerpo =
	 * Metavariables.remplazarMetavariables(cuerpo, reserva, formatoFecha,
	 * formatoHora, "", ""); MailUtiles.enviarMail(emailTo, asunto, cuerpo,
	 * MailUtiles.CONTENT_TYPE_HTML); } Comunicacion comunicacion = new
	 * Comunicacion(Tipo1.EMAIL, Tipo2.CANCELA, emailTo, recurso, reserva,
	 * asunto+"/"+cuerpo); comunicacion.setProcesado(true);
	 * entityManager.persist(comunicacion); }else { //Indicar que no se pudo enviar
	 * el mail return false; } //Indicar que se pudo enviar el mail return true;
	 * }catch(MessagingException mEx) { //Indicar que no se pudo enviar el mail
	 * logger.warn("No se pudo enviar el mail de cancelación", mEx); return false; }
	 * }
	 */
	/*
	 * private boolean almacenarSmsYTav(Comunicacion.Tipo2 tipo2, Reserva reserva,
	 * String formatoFecha, String formatoHora) { //Se envía el mail obligatorio al
	 * usuario try { Recurso recurso =
	 * reserva.getDisponibilidades().get(0).getRecurso(); //Obtener los numeros de
	 * telefono del usuario, que son los datos a solicitar llamados "TelefonoMovil"
	 * y "TelefonoFijo" en la agrupacion que no se puede borrar for(DatoReserva dato
	 * : reserva.getDatosReserva()) { DatoASolicitar datoSol =
	 * dato.getDatoASolicitar();
	 * if("TelefonoMovil".equalsIgnoreCase(datoSol.getNombre()) &&
	 * !datoSol.getAgrupacionDato().getBorrarFlag()) { //Tiene telefono movil, se
	 * envia SMS String telefonoMovil = dato.getValor(); Comunicacion comunicacion =
	 * new Comunicacion(Tipo1.SMS, tipo2, telefonoMovil, recurso, reserva, null);
	 * entityManager.persist(comunicacion); }
	 * if("TelefonoFijo".equalsIgnoreCase(datoSol.getNombre()) &&
	 * !datoSol.getAgrupacionDato().getBorrarFlag()) { //Tiene telefono movil, se
	 * envia SMS String telefonoFijo = dato.getValor(); Comunicacion comunicacion =
	 * new Comunicacion(Tipo1.TEXTOAVOZ, tipo2, telefonoFijo, recurso, reserva,
	 * null); entityManager.persist(comunicacion); } } return true; }catch(Exception
	 * ex) { return false; } }
	 */
	/*
	 * private boolean enviarAvisoMiPerfil(Empresa empresa, Reserva reserva,
	 * Comunicacion.Tipo2 tipoAviso) { return miPerfilBean.enviarAviso(empresa,
	 * reserva, tipoAviso); }
	 */
	@SuppressWarnings("unchecked")
	@Override
	public Map<String, String> consultarTextos(String idioma) throws ApplicationException {
		...
		try {
			...
			List<TextoGlobal> tGlobales = globalEntityManager.createQuery("SELECT t from TextoGlobal t ")
					.getResultList();
			if (tGlobales != null) {
		...
		// Luego se cargan los textos sobreescritos por el tenant pudiendo pisar textos
		// cargados antes
		// Atención: es probable que no haya aún un tennant configurado y por tanto no
		// se puede hacer esta parte
		// (pero no hay forma de determinarlo)
		try {
			List<TextoTenant> tTenants = entityManager
	...
	public Map<String, String> consultarPreguntasCaptcha(String idioma) throws ApplicationException {
		Map<String, String> preguntasCaptcha = new HashMap<>();
		try {
			List<PreguntaCaptcha> pcs = entityManager
	...
	public List<TramiteAgenda> consultarTramites(Agenda agenda) throws ApplicationException {
		try {
			List<TramiteAgenda> tramites = entityManager
					.createQuery(
							"SELECT t from TramiteAgenda t " + "WHERE t.agenda = :agenda " + "ORDER BY t.tramiteNombre")
	...
	@SuppressWarnings("unchecked")
	/*
	 * Busca un trámite correspondiente a la agenda indicada con el código dado. No
	 * debería pasar que haya más de uno, pero si lo hay devuelve el más nuevo
	 * (mayor id).
	 *
	 * @param agenda
	 * 
	...
	/*
	 * Confirma la reserva. Si la reserva debe tener datos, estos son exigidos y
	 * validados en este metodo, incluyendo la verificacion de clave unica
	 *
	 * @throws ApplicationException
	 * 
	 * @throws BusinessException
	...
	public Reserva confirmarReservaPresencial(Empresa empresa, Reserva reserva,
			InformacionRequestDTO informacionRequestDTO) throws ApplicationException, BusinessException,
			ValidacionException, AccesoMultipleException, UserException {
		if (reserva == null || reserva.getDatosReserva() == null) {
			throw new BusinessException("debe_especificar_la_reserva");
		}
		// Esto se lee antes de recuperar la reserva de la base porque puede haber
		...
		// Confirmo la reserva, paso el estado a Reservada y le asigno el numero de
		// reserva dentro de la disponibilidad.
		// Con mutua exclusion en el acceso al numerador de la disponibilidad
		Disponibilidad disponibilidad = reserva.getDisponibilidades().get(0);
		try {
			disponibilidad.setNumerador(disponibilidad.getNumerador() + 1);
		...
		if (empresa.getOid() != null && transaccionId != null) {
			...
			String trazaGuid = trazaBean.registrarCabezal(empresa, reserva, transaccionId, reserva.getTramiteCodigo(),
					true, null, null, informacionRequestDTO);
			if (trazaGuid != null) {
				reserva.setTrazabilidadGuid(trazaGuid);
			} else {
				reserva.setTrazabilidadGuid("---");
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<Integer> cancelarReservasPeriodo(Empresa empresa, Recurso recurso, VentanaDeTiempo ventana,
			String idioma, String formatoFecha, String formatoHora, String asunto, String cuerpo) throws UserException {
		// Obtener las reservas en estado Pendiente, Reservado o Usado para el período
		// indicado
		List<Reserva> reservas = entityManager
				.createQuery("SELECT r FROM Reserva r " + "JOIN r.disponibilidades d "
						+ "WHERE d.recurso.id = :recursoId " + "  AND r.estado IN ('P', 'R', 'U') "
						+ "  AND d.fecha BETWEEN :fi AND :ff " + "  AND d.presencial = false ")
				.setParameter("recursoId", recurso.getId()).setParameter("fi", ventana.getFechaInicial())
				.setParameter("ff", ventana.getFechaFinal()).getResultList();
		List<Integer> reservasSinEnviarComunicacion = new ArrayList<>();
		// Para cada reserva encontrada, cancelarla, enviando las comunicaciones
		// correspondientes
		for (Reserva reserva : reservas) {
			...
			if (!estadoReserva.equals(Estado.P)) {
				try {
					comunicacionesEJB.enviarComunicacionesCancelacion(empresa, reserva, idioma, formatoFecha,
							formatoHora, asunto, cuerpo);
				} catch (Exception ex) {
	...
	/*
	 * Este método es utilizado para marcar la reserva y confirmarla en un solo
	 * paso. Está pensado para ser invocado mediante el servicio web REST
	 * confirmarReserva.
	 *
	 * @return
	 * 
	...
	@Override
	@SuppressWarnings("UseSpecificCatch")
	public Reserva generarYConfirmarReserva(Integer idEmpresa, Integer idAgenda, Integer idRecurso,
			Integer idDisponibilidad, String valoresCampos, String idTransaccionPadre, String pasoTransaccionPadre,
			TokenReserva tokenReserva, String idioma, OrigenReserva origen, InformacionRequestDTO informacionRequestDTO)
			throws UserException {
		if (idEmpresa == null) {
			throw new UserException("debe_especificar_la_empresa");
		}
		...
		Disponibilidad disponibilidad = entityManager.find(Disponibilidad.class, idDisponibilidad);
		if (disponibilidad == null || !disponibilidad.getRecurso().getId().equals(idRecurso)) {
			throw new UserException("no_se_encuentra_la_disponibilidad_especificada");
		}
		// Marcar la reserva
		Reserva reserva = marcarReserva(disponibilidad, origen, tokenReserva, null);
		// Cargar las agrupaciones
		List<AgrupacionDato> agrupaciones = recursosEJB.consultarDefinicionDeCampos(disponibilidad.getRecurso(),
				timezone);
		// Cargar los datos en la reserva; la respuesta puede contener datos que no
		// están dentro de los datos a solicitar del recurso
		...
		// Cargar los datos del trámite si fueron incluidos (deben tener como clave
		// tramite.codigo y opcionalmente tramite.nombre)
	...
	/*
	 * Carga los datos de la reserva obtenidos por parámetro en la reserva. Los
	 * datos deben estar en el mismo formato en el que son pasados por GET como
	 * valores por defecto para los campos, es decir [agrupacion.campo.valor;]*. Los
	 * datos son cargados directamente en la reserva, excepto en los casos en que la
	 * clave agrupacion.campo no se encuentre en la lista de datos a solicitar del
	 * recurso, en cuyo caso se devuelven en el resultado.
	 *
	 * @param reserva
	 * 
	 * @param agrupaciones
	 ...
	 * @return
	 */
	private Map<String, String> cargarDatosReserva(Reserva reserva, List<AgrupacionDato> agrupaciones,
			String valoresCampos) {
		// Lista de datos obtenidos por parámetro que no están dentro de los datos a
		// solicitar del recurso
		// (Por ejemplo, el código y nombre del trámite (tramite.codigo y
		// tramite.nombre)
		Map<String, String> datosAdicionales = new HashMap<>();
		// Armar dos mapas con los datos a solicitar, uno por ids y otro por nombre para
		// accederlos más rápido
		...
		// Procesar cada uno de los parámetros, que deberían ser de la forma
		// <agrupacion>.<dato>.<valor>
		for (String parm : parametros) {
			...
			if (agrupCampoValor.length == 3) {
				...
				// Si tanto la agrupacion como el dato son numéricos se asume que son los ids,
				// sino se asume que son los nombres
	...
	@Override
	public TokenReserva generarTokenReserva(Integer idRecurso, String cedula, String nombre, String correoe,
			String codigoTramite, String ipOrigen) {
	...
	public TokenReserva obtenerTokenReserva(String token) {
		// Solo debería haber uno, pero por las dudas se ordena por id y se queda con el
		// último
		String eql = "SELECT t FROM TokenReserva t WHERE t.token=:token ORDER BY t.id DESC";
		List<TokenReserva> tokens = entityManager.createQuery(eql).setParameter("token", token).getResultList();
		if (!tokens.isEmpty()) {
			return tokens.get(0);
		}
		return null;
	...
	/*
	 * Devuelve todas las reservas asociadas al token indicado. Si el parámetro
	 * incluirIncompletas es false solo incluye las que tengan al menos los datos
	 * obligatorios completos.
	 */
	@SuppressWarnings("unchecked")
	@Override
	public List<Reserva> obtenerReservasMultiples(Integer tokenId, boolean incluirIncompletas) {
		String eql = "SELECT r FROM Reserva r WHERE r.token.id=:tokenId"
				+ (incluirIncompletas ? "" : " AND r.datosReserva IS NOT EMPTY") + " ORDER BY r.id";
		return entityManager.createQuery(eql).setParameter("tokenId", tokenId).getResultList();
	}

	/*
	 * Confirma todas las reservas asociadas al token que tengan al menos los datos
	 * obligatorios completos. Las reservas que no tienen los datos obligatorios
	 * completos son eliminadas.
	 */
	@SuppressWarnings("unchecked")
	@Override
	@TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
	public TokenReserva confirmarReservasMultiples(Integer empresaId, Integer tokenId, String transaccionPadreId,
			Long pasoPadre, boolean inicioAsistido, InformacionRequestDTO informacionRequestDTO) throws UserException {
		if (tokenId == null) {
		...
		List<Reserva> reservas = entityManager.createQuery(eql).setParameter("tokenId", tokenId).getResultList();
		...
		for (Reserva reserva : reservas) {
			// Si la reserva tiene datos se confirma; si no tiene datos (quedó incompleta:
			// eligió fecha y hora pero no completó datos) se borra
			if (!reserva.getDatosReserva().isEmpty()) {
				...
				if (reserva.getTramiteNombre() == null && reserva.getTramiteCodigo() != null) {
					try {
						TramiteAgenda tramite = consultarTramitePorCodigo(
								reserva.getDisponibilidades().get(0).getRecurso().getAgenda(),
								reserva.getTramiteCodigo());
						if (tramite != null) {
				...
				if (empresa.getOid() != null && transaccionId != null) {
					...
					// solo habría que guardar la invocación
					// en la base de datos para futuros intentos
					trazaBean.registrarLinea(empresa, reserva, transaccionId,
							reserva.getDisponibilidades().get(0).getRecurso().getNombre(),
							ServiciosTrazabilidadBean.Paso.RESERVA);
					// Publicar la novedad
					novedadesBean.publicarNovedad(empresa, reserva, Acciones.RESERVA);
				}
	...
	public TokenReserva cancelarReservaMultiple(Integer tokenId, Integer reservaId) throws UserException {
		...
		if (reserva == null || reserva.getToken() == null
				|| !reserva.getToken().getToken().equals(tokenReserva.getToken())) {
			throw new UserException("no_se_encuentra_la_reserva_especificada");
		}
	...
	/*
	 * Este método es utilizado para modificar una reserva. Solo se permite
	 * modificar la disponibilidad, no el recurso porque los datos a solicitar
	 * podrían ser diferentes ni los datos ingresados porque podrían ser de otra
	 * persona. Está pensado para ser invocado mediante el servicio web REST
	 * modificarReserva.
	 *
	 * @return
	 * 
	...
	@Override
	@SuppressWarnings("UseSpecificCatch")
	public Reserva modificarReserva(Integer idEmpresa, Integer idAgenda, Integer idRecurso, Integer idReserva,
			Integer idDisponibilidad, TokenReserva tokenReserva, String idioma, String ipOrigen, OrigenReserva origen)
			throws UserException {
		if (idEmpresa == null) {
			throw new UserException("debe_especificar_la_empresa");
		}
		...
		// Obtener la reserva
		Reserva reservaOriginal = entityManager.find(Reserva.class, idReserva);
		...
		// Marcar la reserva
		Reserva reservaNueva = marcarReservaValidandoDatos(disponibilidad, origen, reservaOriginal, null, ipOrigen);
		// Si pudo hacer eso, la disponibilidad tiene cupo, se puede apuntar la reserva
		// existente a ella y eliminar la reserva dummy
	...
	@Override
	@SuppressWarnings("UseSpecificCatch")
	public Reserva modificarReserva(Integer idEmpresa, Integer idAgenda, Integer idRecurso, Integer idReservaOriginal,
			Integer idReservaNueva, String idioma) throws UserException {
		if (idEmpresa == null) {
			throw new UserException("debe_especificar_la_empresa");
		}
		if (idAgenda == null) {
		...
		Reserva reservaOriginal = entityManager.find(Reserva.class, idReservaOriginal);
		if (reservaOriginal == null) {
		...
		Reserva reservaNueva = entityManager.find(Reserva.class, idReservaNueva);
	...
	private Reserva actualizarReserva(Empresa empresa, Reserva reserva, String transaccionPadreId, Long pasoPadre,
			boolean inicioAsistido) throws ApplicationException, BusinessException, ValidacionException,
			AccesoMultipleException, UserException {
		// Confirmo la reserva, paso el estado a Reservada (si no es múltiple) y le
		// asigno el numero de reserva dentro de la disponibilidad.
		// Con mutua exclusion en el acceso al numerador de la disponibilidad
		Disponibilidad disponibilidad = reserva.getDisponibilidades().get(0);
		...
		// Registrar en el sistema de trazas una línea para la cancelación de la vieja y
		// otra para la confirmación de la nueva
		String transaccionId = trazaBean.armarTransaccionId(empresa.getOid(), reserva.getTramiteCodigo(),
				reserva.getId());
		if (empresa.getOid() != null && transaccionId != null) {
			trazaBean.registrarLinea(empresa, reserva, transaccionId, recurso.getNombre(),
	...
	public void modificarReservaMultipleNotificar(TokenReserva tokenReserva, Boolean notificar) throws UserException {
		...
		@SuppressWarnings("unchecked")
		List<Reserva> reservas = entityManager.createQuery(eql).setParameter("tokenId", tokenReserva.getId())
				.getResultList();
	 ...
	 * Para controlar la existencia de cupo sin necesidad de utilizar bloqueo
	 * persiste la reserva y luego chequea que el cupo real no sea negativo, si esto
	 * sucede, elimina fisicamente la reserva y cancela la operacion. En este caso
	 * se hará el mismo procedimiento para la reserva dos, solo que si no hay
	 * disponibilidad para la reserva uno ni para la reserva dos, se eliminan las
	 * dos reservas
	 * 
	 * @throws BusinessException
	 * 
	 ...
	 */
	@Override
	public Reserva marcarReservasPares(Disponibilidad disponibilidad, Disponibilidad disponibilidad2,
			OrigenReserva origen, TokenReserva token, String ipOrigen) throws UserException {
		if (disponibilidad == null) {
			throw new UserException("debe_especificar_la_disponibilidad");
		}
		disponibilidad = entityManager.find(Disponibilidad.class, disponibilidad.getId());
		...
		try {
			// Se crean las dos reserva para primera y segunda dosis, en una transaccion
			// independiente
			reserva = helper.crearReservasParesPendiente(disponibilidad, disponibilidad2, origen, token, ipOrigen);

			// Chequeo que el cupo real no de negativo tanto en la reserva de la dosis uno
			// como en la dosis dos
			// Si alguno de los cupo reales da negativo, elimino la dos reservas pendiente y
			// cancelo la operacion
			// De lo contrario las reservas se han marcado con exito
			if (helper.chequeoCupoDisponible(disponibilidad, true)
					|| helper.chequeoCupoDisponible(disponibilidad2, true)) {
	 ...
	 */
	@Override
	public Reserva confirmarReservasPares(Empresa empresa, Reserva reserva, String transaccionPadreId, Long pasoPadre,
			boolean inicioAsistido) throws ApplicationException, BusinessException, ValidacionException,
			AccesoMultipleException, UserException {
...
//        
	...
	/*
	 * Este método es utilizado para marcar las dos reservas de vacunación y
	 * confirmarlas en un solo paso. Está pensado para ser invocado mediante el
	 * servicio web REST confirmarReserva.
	 *
	 * @return
	 * 
	 * @throws UserException
	...
	@Override
	@SuppressWarnings("UseSpecificCatch")
	public Reserva generarYConfirmarReservasVacunacion(Integer idEmpresa, Integer idAgenda, Integer idRecurso,
			Integer idDisponibilidad, String valoresCampos, String idTransaccionPadre, String pasoTransaccionPadre,
			TokenReserva tokenReserva, String idioma, OrigenReserva origen, Date fechaReservaDos,
			String tipoDocReservaDos, String tipoDosisReservaDos) throws UserException {
		...
		Disponibilidad disponibilidad2 = disponibilidadEJB
				.obtenerDisponibilidadEnHoraInicio(disponibilidad.getRecurso(), fechaReservaDos);
		if (disponibilidad2 == null || !disponibilidad2.getRecurso().getId().equals(idRecurso)) {
			throw new UserException("no_se_encuentra_la_disponibilidad2_especificada");
		}

		if (disponibilidad2.getHoraInicio().before(disponibilidad.getHoraInicio())) {
			throw new UserException("la_fecha_reserva_dos_debe_ser_posterior_a_la_fecha_reserva_uno");
		}

		// Marcar la reserva
		Reserva reserva = marcarReservasPares(disponibilidad, disponibilidad2, origen, tokenReserva, null);
		// Cargar las agrupaciones
		List<AgrupacionDato> agrupaciones = recursosEJB.consultarDefinicionDeCampos(disponibilidad.getRecurso(),
				timezone);
		// Cargar los datos en la reserva; la respuesta puede contener datos que no
		// están dentro de los datos a solicitar del recurso
		Map<String, String> datosAdicionales = cargarDatosReservaPares(reserva, agrupaciones, valoresCampos,
				tipoDocReservaDos, tipoDosis);
		// Cargar los datos de trazabilidad
		Long pasoPadre = null;
		if (pasoTransaccionPadre != null) {
			try {
		...
		try {
			...
			while (!confirmada) {
				try {
					Reserva rConfirmada = confirmarReservasPares(empresa, reserva, idTransaccionPadre, pasoPadre,
							false);
					reserva.setSerie(rConfirmada.getSerie());
					reserva.setNumero(rConfirmada.getNumero());
					reserva.setCodigoSeguridad(rConfirmada.getCodigoSeguridad());
	 ...
	 * @return
	 */
	private Map<String, String> cargarDatosReservaPares(Reserva reserva, List<AgrupacionDato> agrupaciones,
			String valoresCampos, String tipoDocReserva2, String tipoDosisReserva2) {
		// Lista de datos obtenidos por parámetro que no están dentro de los datos a
		// solicitar del recurso
		// (Por ejemplo, el código y nombre del trámite (tramite.codigo y
		// tramite.nombre)
		...
		// Obtener las tres partes (agrupación, dato, valor) de tipo documento y tipo
		// dosis para la reserva 2
		...
		for (String parm : parametros) {
			...
			if (agrupCampoValor.length == 3 && agrupCampoValorReserva2.length == 3) {
				...
				if (datoASolicitar != null) {
					...
					if (agrupCampoValorReserva2.length == 3 && datoASolicitar
							.equals(porNombre.get(agrupCampoValorReserva2[0] + "." + agrupCampoValorReserva2[1]))) {
	...
	public TokenReserva cancelarReservasParesMultiple(Integer tokenId, Integer reservaId) throws UserException {
		...
		// Pregunta si la reserva tiene otra reserva asociada, si fue en pares por
		// vacunación
		if (reserva.getReservaHija() != null) {
			reserva2 = entityManager.find(Reserva.class, reserva.getReservaHija().getId());
			if (reserva2 == null || reserva2.getToken() == null
					|| !reserva2.getToken().getToken().equals(tokenReserva.getToken())) {
				throw new UserException("no_se_encuentra_la_reserva_especificada");
			}
		}
	...
	@Override
	public void cancelarReservaVacunacion(Integer idEmpresa, Integer idAgenda, Integer idRecurso, Integer idReserva,
			Integer idReserva2, Boolean masiva) throws UserException {
		if (idEmpresa == null) {
			throw new UserException("debe_especificar_la_empresa");
		}
		if (idAgenda == null) {
		...
		Reserva reserva2 = entityManager.find(Reserva.class, idReserva2);
		if (reserva == null) {
			throw new UserException("no_se_encuentra_la_reserva_o_ya_fue_cancelada");
		}
		if (!Estado.R.equals(reserva.getEstado())) {
		...
		for (Reserva r : reservas) {
			...
			reservaDTO.setTramiteCodigo(r.getTramiteCodigo());
			reservaDTO.setTramiteNombre(r.getTramiteNombre());
			reservaDTO.setUcancela(ctx.getCallerPrincipal().getName().toLowerCase()); // el usuario de sesion es el que
																						// esta cancelando la reserva
																						// anterior y ese dato lo
																						// enviamos hacia GAP
			reservaDTO.setTcancela(masiva ? TipoCancelacion.M.toString() : TipoCancelacion.I.toString());
			reservaDTO.setFcancela(new Date());
			if (r.getLlamada() != null) {
			...
			String transaccionId = trazaBean.armarTransaccionId(empresa.getOid(), r.getTramiteCodigo(), r.getId());
			if (transaccionId != null) {
				trazaBean.registrarLinea(empresa, r, transaccionId, recurso.getNombre(),
						ServiciosTrazabilidadBean.Paso.CANCELACION);
			}
			// Publicar la novedad
	...
	@Override
	public ResultadoEjecucion validarMoverReservas(Empresa empresa, Recurso recursoOrigen, Recurso recursoDestino,
			VentanaDeTiempo ventanaOrigen, VentanaDeTiempo ventanaDestino)
			throws UserException, ApplicationException, BusinessException {
		ResultadoEjecucion ret = new ResultadoEjecucion();

		// Refrescar los recursos para que carguen los datos a solicitar y las
		// disponibilidades
		...
		if (recursoOrigen.equals(recursoDestino)) {
			...
			// ======================================================================================================
			// Verificar si la fecha inicio destino está dentro de la ventana del recurso
			// origen, se traslapa
			if (ventanaDestino.getFechaInicial().after(ventanaOrigen.getFechaInicial())
					&& ventanaDestino.getFechaInicial().before(ventanaOrigen.getFechaFinal())) {
				...
				// Para cada disponibilidad en el origen verificar si existe en el destino y si
				// tiene cupos disponibles suficientes para las reservas
				List<Object[]> reservasConfirmadasyPendientesOrigen = obtenerReservasConfirmadasPorDisponibilidad(
						recursoOrigen, ventanaOrigen);
				...
				if (!cuposDestino.isEmpty()) {
					...
					for (Object[] reservaOrigen : reservasConfirmadasyPendientesOrigen) {
						reservasPorHoraInicio.put(formatoHora.format((Date) reservaOrigen[0]),
								((BigInteger) reservaOrigen[2]).intValue());
					...
					for (Object[] cupoDestino : cuposDestino) {
						...
						if (reservasPorHoraInicio.get(formatoHora.format((Date) cupoDestino[1])) != null
								&& reservasPorHoraInicio.get(formatoHora.format((Date) cupoDestino[1])) != 0) {
							cuposDisponibles = cuposDisponibles
									+ reservasPorHoraInicio.get(formatoHora.format((Date) cupoDestino[1]));
						}
					}
					...
					if (cuposDisponibles >= reservas) {
						...
						ret.getErrores().add(
								"No existen cupos suficientes para las reservas que se desean mover, teniendo en cuenta los intervalos mínimos entre horarios.");
					}
				} else {
					ret.getErrores().add(
							"No existen disponibilidades a partir del horario de inicio del recurso destino para las reservas que se desean mover");
				}
			} else {
				...
				// Para cada disponibilidad en el origen verificar si existe en el destino y si
				// tiene cupos disponibles suficientes para las reservas
				List<Object[]> reservasConfirmadasyPendientesOrigen = obtenerReservasConfirmadasPorFrecuencia(
						recursoOrigen, ventanaOrigen);

				// vamos a crear un maps uno para el recurso destino con frecuencia ->cupos
				// disponible
				...
				if (!cuposDestino.isEmpty()) {
					...
					for (Object[] cupoDestino : cuposDestino) {
						cuposDisponiblesPorFrecuencia.put(((Integer) cupoDestino[0]).toString(),
								((Integer) cupoDestino[1]).intValue());
					}

					for (Object[] reservaOrigen : reservasConfirmadasyPendientesOrigen) {

						Integer cupoDisponible = cuposDisponiblesPorFrecuencia
								.get(((Integer) reservaOrigen[0]).toString());
						if (cupoDisponible != null && ((Integer) reservaOrigen[1]) != null) {
							if (cupoDisponible >= (Integer) reservaOrigen[1]) {
								...
								cantidadReservasTotalesAMover = cantidadReservasTotalesAMover
										+ ((Integer) reservaOrigen[1]).intValue();
							} else {
			...
			// Validaciones previas (algunas son críticas otras son advertencias y queda a
			// criterio del usuario continuar)
			// ======================================================================================================
			// Período de validación y política de reservas múltiples
			// El período de validación debería ser compatible (el destino debe ser el
			// mismo)
			if (recursoDestino.getPeriodoValidacion().intValue() == recursoOrigen.getPeriodoValidacion().intValue()) {
				ret.getMensajes()
						.add("El período de validación del recurso destino es compatible con el del recurso origen.");
			} else {
				ret.getErrores()
						.add("El período de validación del recurso destino no es el mismo que el del recurso origen.");
			}
			// La aceptación o no de reservas múltiples debería ser el mismo (ambos aceptan
			// o ninguno acepta)
			if (recursoDestino.getMultipleAdmite() != recursoOrigen.getMultipleAdmite()) {
				ret.getErrores().add(
						"La política de aceptación de reservas múltiples no es la misma en el recurso origen y en el recurso destino.");
			} else {
				ret.getMensajes().add(
						"La política de aceptación de reservas múltiples del recurso destino es compatible con la del recurso origen.");
			}
			// ======================================================================================================
			// Datos a solicitar (claves, obligatorios, tipos de datos)
			// Los datos a solicitar clave en el recurso destino deben existir en el recurso
			// origen y deberían ser clave también
			// Todos los datos a solicitar no-clave pero obligatorios en el recurso destino
			// deben existir en el recurso origen y deberían ser obligatorios
			...
			for (DatoASolicitar datoSolicitarDestino : recursosEJB.consultarDatosSolicitar(recursoDestino)) {
				...
				for (DatoASolicitar datoSolicitarOrigen0 : recursosEJB.consultarDatosSolicitar(recursoOrigen)) {
					if (datoSolicitarOrigen0.getNombre().equals(datoSolicitarDestino.getNombre())
							&& datoSolicitarOrigen0.getAgrupacionDato().getNombre()
									.equals(datoSolicitarDestino.getAgrupacionDato().getNombre())) {
				...
				if (datoSolicitarOrigen != null) {
					if (datoSolicitarDestino.getEsClave()) {
						...
						if (!datoSolicitarOrigen.getEsClave()) {
							...
							ret.getErrores()
									.add("El dato '" + datoSolicitarDestino.getAgrupacionDato().getEtiqueta() + "->"
											+ datoSolicitarDestino.getEtiqueta() + "' "
					...
					if (datoSolicitarDestino.getRequerido()) {
						if (!datoSolicitarOrigen.getRequerido()) {
							...
							ret.getErrores().add("El dato '" + datoSolicitarDestino.getAgrupacionDato().getEtiqueta()
									+ "->" + datoSolicitarDestino.getEtiqueta() + "' "
					...
					if (datoSolicitarDestino.getIncluirEnNovedades()) {
						// El dato es incluido en novedades en el recurso origen pero no en el destino
						// (podría causar duplicaciones al mover reservas)
				...
				if (datoSolicitarOrigen != null) {
					...
					if (datoSolicitarDestino.getTipo() != datoSolicitarOrigen.getTipo()) {
						ret.getErrores()
								.add("El tipo del dato '" + datoSolicitarDestino.getAgrupacionDato().getEtiqueta()
										+ "->" + datoSolicitarDestino.getEtiqueta() + "' "
						...
						case STRING:
						case NUMBER:
							// El largo máximo en el destino debe ser igual o mayor al largo máximo en el
							// origen
							if (datoSolicitarDestino.getLargo() < datoSolicitarOrigen.getLargo()) {
								ret.getErrores().add("El tipo del dato '"
										+ datoSolicitarDestino.getAgrupacionDato().getEtiqueta() + "->"
										+ datoSolicitarDestino.getEtiqueta() + "' "
			...
			for (DatoASolicitar datoSolicitarOrigen : recursosEJB.consultarDatosSolicitar(recursoOrigen)) {
				...
				for (DatoASolicitar datoSolicitarDestino0 : recursosEJB.consultarDatosSolicitar(recursoDestino)) {
					if (datoSolicitarDestino0.getNombre().equals(datoSolicitarOrigen.getNombre())
							&& datoSolicitarDestino0.getAgrupacionDato().getNombre()
									.equals(datoSolicitarOrigen.getAgrupacionDato().getNombre())) {
				...
				if (datoSolicitarDestino != null) {
					...
					if (datoSolicitarOrigen.getEsClave()) {
						// El dato es clave en el recurso origen pero no en el destino (podría causar
						// duplicaciones al mover reservas)
						if (!datoSolicitarDestino.getEsClave()) {
							ret.getErrores()
									.add("El dato '" + datoSolicitarOrigen.getAgrupacionDato().getEtiqueta() + "->"
											+ datoSolicitarOrigen.getEtiqueta() + "' "
											+ "es clave en el recurso origen pero no es clave en el recurso destino.");
						}
					}
					if (datoSolicitarOrigen.getRequerido()) {
						// El dato es requerido en el recurso origen pero no en el destino (podría
						// causar duplicaciones al mover reservas)
						if (!datoSolicitarDestino.getRequerido()) {
							ret.getErrores().add("El dato '" + datoSolicitarOrigen.getAgrupacionDato().getEtiqueta()
									+ "->" + datoSolicitarOrigen.getEtiqueta() + "' "
			...
			// Si no se añadió ningún mensaje nuevo por motivo de los datos a solicitar, es
			// porque todos están OK
			if (cantidadErroresAntes == ret.getErrores().size() && cantidadWarningsAntes == ret.getWarnings().size()) {
				ret.getMensajes().add(
						"No hay incompatibilidades entre los datos a solicitar en el recurso destino y el recurso origen.");
			}
			// ======================================================================================================
			// Disponibilidades y cupos
			// se obtienen las disponibilidades del destino con la frecuencia
			...
			// Para cada disponibilidad en el origen verificar si existe en el destino y si
			// tiene cupos disponibles suficientes para las reservas
			List<Object[]> reservasConfirmadasyPendientesOrigen = obtenerReservasConfirmadasPorFrecuencia(recursoOrigen,
					ventanaOrigen);
			// vamos a crear un maps uno para el recurso destino con frecuencia ->cupos
			// disponible
			Map<String, Integer> cuposDisponiblesPorFrecuencia = new HashMap<String, Integer>();
	...
	@Override
	public ResultadoEjecucion ejecutarMoverReservas(Empresa empresa, Recurso recursoOrigen, Recurso recursoDestino,
			VentanaDeTiempo ventanaOrigen, VentanaDeTiempo ventanaDestino, boolean enviarComunicaciones,
			String linkBase, boolean generarNovedades, String uuid)
			throws UserException, ApplicationException, BusinessException {
		ResultadoEjecucion ret = new ResultadoEjecucion();
		...
		try {
			...
			if (recursoOrigen.equals(recursoDestino)) {
				...
				if (recursoOrigen.getAgenda().getTimezone() != null
						&& !recursoOrigen.getAgenda().getTimezone().isEmpty()) {
				...
				// sabiendo que son el mismo recurso, algunas de esas disponibilidades pueden ya
				// existir si se traslapan
				List<Disponibilidad> disponibilidadesOrigen = obtenerDisponibilidades(recursoOrigen, ventanaOrigen,
						timezoneOrigen, false);

				List<Disponibilidad> disponibilidadesDestino = obtenerDisponibilidades(recursoDestino, ventanaDestino,
						timezoneOrigen, false);
				...
				for (Disponibilidad disponibilidadDestino : disponibilidadesDestino) {
					disponibilidadesDestinoPorHora.put(formatoClave.format(disponibilidadDestino.getHoraInicio()),
							disponibilidadDestino);
				}

				// Armar un mapeo entre los datos a solicitar en el recurso origen y los del
				// recurso destino
				...
				for (DatoASolicitar datoSolicitarDestino : recursosEJB.consultarDatosSolicitar(recursoDestino)) {
					String claveDato = datoSolicitarDestino.getAgrupacionDato().getNombre() + ":"
							+ datoSolicitarDestino.getNombre();
				...
				if (traslape) {
					for (Disponibilidad disponibilidadOrigen : disponibilidadesOrigen) {
						Disponibilidad dispOrigen = entityManager.find(Disponibilidad.class,
								disponibilidadOrigen.getId());
				...
				for (Disponibilidad disponibilidadOrigen : disponibilidadesOrigen) {
					...
					// logger.info("DISPONIBILIDAD ORIGEN " + dispOrigen.getId() + "HORA INICIO " +
					// dispOrigen.getHoraInicio() + "CUPO " + dispOrigen.getCupo());
					...
					if (!reservasOrigen.isEmpty()) {
						...
						for (Disponibilidad disponibilidadDestino : disponibilidadesDestino) {
							...
							Disponibilidad dispoDestino = entityManager.find(Disponibilidad.class,
									disponibilidadDestino.getId());
							// logger.info("DISPONIBILIDAD DESTINO " + dispoDestino.getId() + "HORA INICIO "
							// + dispoDestino.getHoraInicio());

							// Preguntar si las reservasOrigen caben en la disponibilidad destino en la
							// primera que quepa se mete y luego se hace break;
							if (dispOrigen.getFrecuencia() == dispoDestino.getFrecuencia()) {
								// Verificar los cupos en destino donde quepan todas las reservas de la
								// disponibilidad origen
								// aqui antes de preguntar si el cupo es mayor a 0, sumar en dado caso que se
								// traslapen
								if (traslape
										&& dispOrigen.getHoraInicio().compareTo(dispoDestino.getHoraInicio()) == 0) {
									disponibilidadDestino
											.setCupo(disponibilidadDestino.getCupo() + dispOrigen.getReservas().size());
								}
								...
								if (disponibilidadDestino.getCupo() > 0) {
									...
									for (Reserva reservaOrigen : reservasOrigen) {
										...
										if (reservaOrigen.getEstado() == Estado.R
												&& disponibilidadDestino.getCupo() > 0) {
											if (dispoDestino == null) {
												// No debería haber llegado hasta acá: hay reservas en el origen pero no
												// existe la disponibilidad en el destino
												throw new BusinessException("no_se_puedo_mover_las_reservas");
											}
											// Mover la reserva
											Reserva reservaDestino = moverReserva(reservaOrigen, dispoDestino,
													datosSolicitarPorClave);
											// Registrar las reservas a cancelar
											reservaOrigen.setIdDestino(reservaDestino.getId());
											reservasACancelar.add(reservaOrigen);
											// Registrar las reservas (origen y destino) para enviar las comunicaciones
											// al final
									...
									for (Reserva reservaCancelar : reservasACancelar) {
										...
										String observacionOrigen = "Reserva movida a "
												+ dispoDestino.getRecurso().getAgenda().getNombre() + "/"
												+ dispoDestino.getRecurso().getNombre() + "; id destino: "
												+ reservaCancelar.getIdDestino();
									...
									// Refrescar la disponibilidad origen porque solo se tiene un stub con datos
									// mínimos
									...
									try {
										...
										Date fechaDestino = formatoFecha
												.parse(formatoFecha.format(dispoDestino.getHoraInicio()));
										Date fechaOrigen = formatoFecha
												.parse(formatoFecha.format(dispOrigen.getHoraInicio()));
										if (fechaDestino.compareTo(fechaOrigen) == 0) {

//                        
											if (traslape && disponibilidadesDestinoPorHora
													.get(formatoClave.format(dispOrigen.getHoraInicio())) == null) {
												// logger.info("cupos cuando se traslapa y las horas inicio no son
												// iguales");
										...
										java.util.logging.Logger.getLogger(AgendarReservasBean.class.getName())
				...
				// Si se procesaron todas las reservas y no se produjo ningún error se generan
				// las novedades y se mandan las comunicaciones
				if (generarNovedades || enviarComunicaciones) {
					for (int i = 0; i < reservasMovidasOrigen.size(); i++) {
						...
						if (enviarComunicaciones) {
							// ToDo: cambiar esto para enviar mail de movimiento en lugar de
							// cancelación/confirmación
							String linkCancelacion = linkBase + "/sae/cancelarReserva/Paso1.xhtml?e=" + empresa.getId()
									+ "&a=" + recursoDestino.getAgenda().getId() + "&ri=" + reservaDestino.getId();
							String linkModificacion = linkBase + "/sae/modificarReserva/Paso1.xhtml?e="
									+ empresa.getId() + "&a=" + recursoDestino.getAgenda().getId() + "&r="
									+ recursoDestino.getId() + "&ri=" + reservaDestino.getId();
							comunicacionesEJB.enviarComunicacionesTraslado(empresa, linkCancelacion, linkModificacion,
									reservaOrigen, reservaDestino, null, empresa.getFormatoFecha(),
...
//            logger.info("reservas movidas destino " + reservasMovidasDestino.size());
					...
					ret.getMensajes()
							.add("Reservas movidas correctamente; cantidad: " + reservasMovidasOrigen.size() + ".");
				}
				...
				if (recursoDestino.getAgenda().getId().equals(recursoOrigen.getAgenda().getId())) {
					...
					if (recursoDestino.getAgenda().getTimezone() != null
							&& !recursoDestino.getAgenda().getTimezone().isEmpty()) {
				...
				for (Disponibilidad disponibilidadOrigen : disponibilidadesOrigen) {
					...
					int reservasReservadas = 0;
					Disponibilidad dispOrigen = entityManager.find(Disponibilidad.class, disponibilidadOrigen.getId());
					// logger.info("DISPONIBILIDAD ORIGEN " + dispOrigen.getId() + " CUPO " +
					// dispOrigen.getCupo());
					...
					if (!reservasOrigen.isEmpty()) {
						...
						for (Disponibilidad disponibilidadDestino : disponibilidadesDestino) {
							...
							// logger.info("DISPONIBILIDAD DESTINO ID " + disponibilidadDestino.getId() + "
							// CUPO " + disponibilidadDestino.getCupo());
							// Preguntar si las reservasOrigen caben en la disponibilidad destino en la
							// primera que quepa se mete y luego se hace break;
							if (dispOrigen.getFrecuencia() == dispoDestino.getFrecuencia()) {
								// Verificar los cupos en destino donde quepan todas las reservas de la
	...
	private List<Object[]> obtenerCuposDisponiblesPorDisponibilidad(Recurso recurso, VentanaDeTiempo periodo) {
		String sQuery = "SELECT ad.id,ad.hora_inicio, ad.cupo - count(ar.id),"
				...
				+ "AND ad.hora_inicio>=:fi AND ad.hora_fin<=:ff " + "GROUP BY ad.id, frecuencia";
	...
	private List<Object[]> obtenerCuposFrecuenciaDisponibles(Recurso recurso, VentanaDeTiempo periodo) {
		String sQuery = "select CAST(t.frecuencia as INTEGER), CAST (sum(t.cupos) as INTEGER) " + "from ( "
				...
				+ "FROM ae_disponibilidades d " + "LEFT JOIN ae_reservas_disponibilidades rd ON rd.aedi_id = d.id "
				...
				+ "AND ad.hora_inicio>=:fi AND ad.hora_fin<=:ff  " + "group by frecuencia,ad.id) as t "
	...
	private List<Object[]> obtenerReservasConfirmadasPorFrecuencia(Recurso recurso, VentanaDeTiempo periodo) {
		String sQuery = "select CAST(t.frecuencia as INTEGER), CAST(sum(t.reservas) as INTEGER) " + "from ( "
				+ "SELECT " + "abs((DATE_PART('day', ad.hora_fin - ad.hora_inicio) * 24 + "
				+ "DATE_PART('hour', ad.hora_fin - ad.hora_inicio)) * 60 + "
				+ "DATE_PART('minute', ad.hora_fin - ad.hora_inicio)) frecuencia, "
				...
				+ "AND ad.fecha_baja is NULL AND ar.estado = 'R' " + "AND ad.hora_inicio>=:fi AND ad.hora_fin<=:ff "
				+ "GROUP BY frecuencia,ad.id) as t " + "GROUP BY t.frecuencia";
	...
	public Long obtenerReservasConfirmadasRecursoOrigen(Recurso recurso, VentanaDeTiempo periodo) throws UserException {
		try {
			...
			String sQuery = "SELECT COUNT(*) FROM Reserva r JOIN r.disponibilidades d WHERE d.recurso.id=:idRecurso "
					+ "AND r.estado = 'R' AND d.fechaBaja is NULL " + "AND d.presencial = false AND d.horaInicio>=:fi "
					+ "AND d.horaFin<=:ff";
	...
	private Reserva moverReserva(Reserva reservaOrigen, Disponibilidad disponibilidadDestino,
			Map<String, DatoASolicitar> datosSolicitarPorClave) throws UserException {
		...
		try {
			...
			// En el caso de que sea parte de una reserva múltiple se reutiliza el mismo
			// token
			// Si necesitan cancelar todo el bloque la nueva reserva debe ser considerada
			// parte del mismo
			...
			// Datos de la reserva (hay que hacerlo después de haber guardado la reserva
			// sino da error)
			for (DatoReserva datoReservaOriginal : reservaOrigen.getDatosReserva()) {
				...
				String claveDato = datoSolicitarOriginal.getAgrupacionDato().getNombre() + ":"
						+ datoSolicitarOriginal.getNombre();
