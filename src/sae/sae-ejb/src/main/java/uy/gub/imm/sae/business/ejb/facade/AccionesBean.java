 ...
 */
@Stateless
public class AccionesBean implements AccionesLocal {
    ...
    /*
     * Crea la accion <b>v</b> en el sistema. Controla la unicidad del nombre de la accion entre todas las acciones vivas (fechaBaja == null). Retorna la accion
     * con su identificador interno. Roles permitidos: Administrador
     *
    ...
    /*
     * Se realiza la baja l�gica de la accion (se setea fechaBaja con la fecha actual del sistema). Controla que no existan datos vivos asociados a la accion ,
     * si es as� no se la da de baja.
     *
     *
     * @throws UserException
     * @throws ApplicationException
    ...
    public void eliminarAccion(Accion v) throws UserException, BusinessException {
        ...
        modificarParametrosAccion(accion, new ArrayList<>());
        accion.setFechaBaja(new Date());
    }

    /*
     * Se realiza la modificación de la accion <b>v</b>. Controla la unicidad del nombre de la accion entre todas las acciones vivas (fechaBaja == null).
     * Actualiza los parametros de acuerdo a los indicados en la accion de entrada. Roles permitidos: Administrador
     *
     * @throws UserException
     * @throws ApplicationException
     */
    ...
    private void modificarParametrosAccion(Accion v, List<ParametroAccion> parametrosModificados) throws UserException {
        ...
        Map<Integer, ParametroAccion> mapModificados = new HashMap<>();
    ...
    private void chequeoNoUsoDelParametro(ParametroAccion p) throws UserException {
        Long cant = (Long) em.createQuery(
                "SELECT COUNT(axd) FROM AccionPorDato axd " +
                " WHERE axd.nombreParametro = :nombre " +
                "   AND axd.accionPorRecurso.accion.id = :accion " +
                "   AND axd.accionPorRecurso.recurso.fechaBaja IS NULL " +
                "   AND axd.fechaDesasociacion IS NULL "
        ).setParameter("nombre", p.getNombre()).setParameter("accion", p.getAccion().getId()).getSingleResult();
        if (cant > 0) {
    ...
    /*
     * Crea una AccionPorDato, es decir, asocia la accion al dato. Controla que esta accion ya no este asociada al dato, es decir que no exista otra
     * AccionPorDato viva para el par accion,dato. Controla que el usuario tenga rol Planificador sobre la agenda asociada Roles permitidos: Planificador
     *
     * @throws UserException
     * @throws ApplicationException
     */
    ...
    private void modificarAccionesPorDatoDeLaAccionPorRecurso(AccionPorRecurso vxr, List<AccionPorDato> accionesPorDatoModificados) throws UserException, BusinessException {
        ...
        Map<Integer, AccionPorDato> mapModificados = new HashMap<>();
    ...
    /*
     * Realiza una baja l�gica de la AccionPorDato (setea fechaBaja a la fecha actual del sistema). Controla que el usuario tenga rol Planificador sobre la
    ...
    private Boolean hayDatosVivos(Accion accion) {
        Long cant = (Long) em.createQuery(
                "SELECT count(axr) " +
                "FROM  AccionPorRecurso axr " +
                "WHERE axr.accion = :accion "+
                "  AND axr.recurso.fechaBaja IS NULL " +
                "  AND axr.fechaBaja is null")
