...
public class DisponibilidadesSingleton {
    ...
    /**
     * Obtiene de la base de datos la instancia de Disponibilidad presencial correspondiente al recurso para la fecha de hoy. Si no existe, crea una y lo
     * almacena (esta es la razón para hacerlo en un Singleton, de forma de que no se creen nunca dos para el mismo día).
     *
     * @param entityManager
     * @param recurso
     * @param timezone
    ...
    public Disponibilidad obtenerDisponibilidadPresencial(EntityManager entityManager, Recurso recurso, TimeZone timezone) {
        ...
        if (recurso == null || recurso.getPresencialAdmite() == null || !recurso.getPresencialAdmite()) {
            return null;
        }
