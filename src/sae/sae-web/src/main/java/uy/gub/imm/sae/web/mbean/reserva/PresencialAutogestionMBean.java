...
public class PresencialAutogestionMBean extends BaseMBean {
        ...
        try {
            ...
            if (datosSolicitar != null) {
                for (DatoASolicitar datoSolicitar : datosSolicitar) {
                    if (!datoSolicitar.getAgrupacionDato().getBorrarFlag() && datoSolicitar.getNombre().equals(DatoASolicitar.TIPO_DOCUMENTO)) {
    ...
    public void solicitarTurno(ActionEvent event) {
        ...
        try {
            ...
            Reserva reserva = agendarReservasEJB.marcarReserva(disponibilidad, OrigenReserva.PRESENCIAL, null, null);
            //Completar los datos de la reserva
            Set<DatoReserva> datosReserva = new HashSet<>();
            if (datosSolicitar != null) {
                for (DatoASolicitar datoSolicitar : datosSolicitar) {
                    ...
                    if (datoSolicitar.getNombre().equals(DatoASolicitar.TIPO_DOCUMENTO)) {
                        datoReserva.setValor(tipoDocumento);
                        datosReserva.add(datoReserva);
                    } else if (datoSolicitar.getNombre().equals(DatoASolicitar.NUMERO_DOCUMENTO)) {
                        ...
                        switch (datoSolicitar.getTipo()) {
                            ...
                            case NUMBER:
                                ...
                                break;
                            case STRING:
                                if (datoSolicitar.getNombre().equals(DatoASolicitar.CORREO_ELECTRONICO)) {
            ...
            while (!confirmada) {
                try {
                    Reserva rConfirmada = agendarReservasEJB.confirmarReservaPresencial(sesionMBean.getEmpresaActual(), reserva, new InformacionRequestDTO(request.getScheme(), request.getServerName(), request.getContextPath(), request.getServerPort()));
