...
public class FormularioDinReservaClient {
    ...
    public static void armarFormularioLecturaDinamico(Recurso recurso, Reserva reserva, UIComponent campos, List<AgrupacionDato> agrupaciones,
            String formatoFecha, Locale locale) throws Exception {
        ...
        if (campos.getChildCount() == 0 && recurso != null && (!reserva.getDatosReserva().isEmpty())) {
            ...
            FormularioDinamicoReserva formularioDin = new FormularioDinamicoReserva(valores, formatoFecha, locale);
    ...
    public static void armarFormularioEdicionDinamico(Recurso recurso, UIComponent filtroConsulta, List<AgrupacionDato> agrupaciones, 
            String formatoFecha, Locale locale)
            throws Exception {
        ...
        if (filtroConsulta.getChildCount() == 0 && recurso != null) {
            FormularioDinamicoReserva formularioDin = new FormularioDinamicoReserva(DATOS_FILTRO_RESERVA_MBEAN, FORMULARIO_ID, 
                    FormularioDinamicoReserva.TipoFormulario.CONSULTA, null, formatoFecha, locale);
            formularioDin.armarFormulario(agrupaciones, null);
            UIComponent formulario = formularioDin.getComponenteFormulario();
    ...
    public static List<DatoReserva> obtenerDatosReserva(Map<String, Object> origen, Map<String, DatoASolicitar> datosASolicitar) throws Exception {
        List<DatoReserva> datos = new ArrayList<>();
