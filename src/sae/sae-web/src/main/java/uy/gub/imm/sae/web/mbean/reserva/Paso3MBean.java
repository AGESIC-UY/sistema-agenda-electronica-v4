...
public class Paso3MBean extends BaseMBean {
    ...
    @PostConstruct
    public void init() throws BusinessException, ApplicationException {
        errorInit = false;
    ...
    public String confirmarReserva() {
        ...
        try {
            ...
            } else if (captchaStyleClass.contains(FormularioDinamicoReserva.STYLE_CLASS_DATO_CON_ERROR)) {
            ...
            while (!confirmada) {
                try {
                    Reserva rConfirmada = agendarReservasEJB.confirmarReserva(sesionMBean.getEmpresaActual(), reserva, transaccionPadreId, pasoPadre, false, new InformacionRequestDTO(request.getScheme(), request.getServerName(), request.getContextPath(), request.getServerPort()));
    ...
    /**
     * El parámetro valoresCampos debe ser una lista separada por punto y coma
     * de elementos de la forma <agrupacion>.<dato>.<valor>, donde <agrupacion>
     * es el identificador o el nombre de la agrupación, <dato> es el
     * identificador o el nombre del dato, y <valor>
     * es el valor a poner en el campo. Notar que el valor no puede contener el
     * signo ";" porque sería interpretado como un cambio de parámetro.
     *
     * @param valoresCampos
     * @param agrupaciones
     */
