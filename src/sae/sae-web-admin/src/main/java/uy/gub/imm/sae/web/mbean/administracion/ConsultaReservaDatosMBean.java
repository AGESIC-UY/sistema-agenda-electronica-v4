...
public class ConsultaReservaDatosMBean extends BaseMBean {
    ...
    @ManagedProperty(value = "#{consReservaDatosSessionMBean}")
    ...
    /**
     * Busca reservas para el recurso actual que coincidan con los datos
     * ingresados por el usuario. No considera reservas que corresponden a
     * disponibilidades presenciales.
     *
     * @param e
     */
    ...
    public String verDetalleReserva(int rowIndex) {
        Reserva reserva = this.consReservaDatosSessionMBean.getListaReservas().get(rowIndex);
        ...
        if (this.sessionMBean.getRecursoMarcado() != null) {
            try {
                ...
                FormularioDinReservaClient.armarFormularioEdicionDinamico(this.sessionMBean.getRecursoMarcado(), filtroConsulta, 
                        agrupaciones, sessionMBean.getFormatoFecha(), new Locale(sessionMBean.getIdiomaActual()));
            } catch (Exception e) {
                addErrorMessage(e);
            }
        }
    ...
    public void setCampos(UIComponent campos) {
        ...
        try {
            ...
            FormularioDinReservaClient.armarFormularioLecturaDinamico(sessionMBean.getRecursoMarcado(), 
                    this.consReservaDatosSessionMBean.getReservaDatos(), this.campos, agrupaciones, sessionMBean.getFormatoFecha(), new Locale(sessionMBean.getIdiomaActual()));
        } catch (Exception e) {
            addErrorMessage(e);
        }
