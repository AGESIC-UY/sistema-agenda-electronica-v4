...
public class ValidacionMantenimientoMBean extends BaseMBean {
    ...
    private static final Logger LOGGER = Logger.getLogger(ValidacionMantenimientoMBean.class.getName());
    
    public static final String MSG_ID = "pantalla";

    @EJB
    private ValidacionesLocal validacionEJB;
    ...
    private void cargarValidaciones() {
        try {
            ...
            validacionesSeleccion = new RowList<>(entidades);
        } catch (Exception e) {
    ...
    public void crearValidacion(ActionEvent event) {
        ...
        this.validacion.setHost("http-remoting://localhost:8080");
    }
    ...
    public void editarValidacion(ActionEvent event) {
        ...
        if (rowSelect != null) {
            ...
            List<ParametroValidacion> parametros = new ArrayList<>();
            try {
                parametros = validacionEJB.consultarParametrosDeLaValidacion(this.validacion);
            } catch (Exception ex) {
                LOGGER.warn("Error al consultar los parámetros de la validación", ex);
    ...
    public void guardarValidacion(ActionEvent event) {
        ...
        try {
            ...
            LOGGER.error("Error al guardar la validación", ex);
            addErrorMessage(ex, MSG_ID);
        }
    }
