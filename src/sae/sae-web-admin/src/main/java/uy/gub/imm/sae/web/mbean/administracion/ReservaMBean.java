...
public class ReservaMBean extends BaseMBean {
    ...
    @ManagedProperty(value = "#{reservaSessionMBean}")
        ...
        if (this.sessionMBean.getRecursoMarcado() != null) {
            try {
                ...
                FormularioDinReservaClient.armarFormularioEdicionDinamico(this.sessionMBean.getRecursoMarcado(), filtroConsulta, agrupaciones, 
                        sessionMBean.getFormatoFecha(), new Locale(sessionMBean.getIdiomaActual()));
    ...
    public void setCampos(UIComponent campos) {
        ...
        try {
            ...
            FormularioDinReservaClient.armarFormularioLecturaDinamico(sessionMBean.getRecursoMarcado(), this.reservaSessionMBean.getReservaDatos(), 
                    this.campos, agrupaciones, sessionMBean.getFormatoFecha(), new Locale(sessionMBean.getIdiomaActual()));
        } catch (BusinessException be) {
            addErrorMessage(be, MSG_ID);
        } catch (Exception e) {
            addErrorMessage(e);
    ...
    /**
     * Cancela todas las reservas existentes en el recurso actual para el
     * período especificado, enviando un mensaje de confirmación a cada uno de
     * los ciudadanos. Además, elimina las disponibilidades existentes en el
     * mismo período para que no se pueda generar otra reserva.
     */
