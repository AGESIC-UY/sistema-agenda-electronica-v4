...
public class ConfiguracionMBean extends BaseMBean {
    ...
    public void beforePhase(PhaseEvent event) {
        if (event.getPhaseId() == PhaseId.RENDER_RESPONSE) {
            sessionMBean.setPantallaTitulo(sessionMBean.getTextos().get("configuracion_del_organismo"));
        }
    }
    ...
    public RowList<Configuracion> getConfiguraciones() {
        try {
            if(sessionMBean.getOrganismoActual()!=null){
                return new RowList<>(configuracionEJB.obtenerConfiguracionesPorOrganismo(sessionMBean.getOrganismoActual().getId()));
            }
        } catch (Exception ex) {
