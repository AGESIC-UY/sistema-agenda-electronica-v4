...
public class DisponibilidadMBean extends BaseMBean {
    ...
    @ManagedProperty(value = "#{dispSessionMBean}")
        ...
        try {
            if (sessionMBean.getRecursoMarcado() != null) {
                if (ventana.getFechaInicial() != null && ventana.getFechaFinal() != null && ventana.getFechaInicial().compareTo(ventana.getFechaFinal()) <= 0) {
                    ...
                    int i = 0;
                    List<CupoPorDia> listaCupos = new ArrayList<>();
                    ...
                    cuposAux = new RowList<>(listaCupos);
                } else {
    ...
    public String configurarDisponibilidadesDelDia(int rowIndex) {
        ...
        CupoPorDia cupoPorDia = dispSessionMBean.getCuposPorDia().get(rowIndex).getData();
        if (cupoPorDia != null) {
            ...
            try {
                ...
                dispSessionMBean.setDisponibilidadesDelDiaMatutina(new RowList<>(dispMatutinas));
                dispSessionMBean.setDisponibilidadesDelDiaVespertina(new RowList<>(dispVespertinas));
    ...
    public void configurarDisponibilidadesDelDiaModif() {
        ...
        dispSessionMBean.setDisponibilidadesDelDiaMatutinaModif(new RowList<>(dispMatutinas));
        dispSessionMBean.setDisponibilidadesDelDiaVespertinaModif(new RowList<>(dispVespertinas));
    }
    ...
    public void guardarModifDisp(ActionEvent event) {
        ...
        if (this.valor == null || this.valor.trim().isEmpty()) {
            ...
            try {
                valorCupo = Integer.valueOf(this.valor);
            } catch (Exception e) {
