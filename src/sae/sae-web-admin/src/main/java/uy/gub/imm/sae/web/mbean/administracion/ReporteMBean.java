...
public class ReporteMBean extends BaseMBean {
    ...
    private List<SelectItem> estadosReserva = new ArrayList<>();
    ...
    private CSVWebFilePrinter printer;
    ...
    public CSVWebFilePrinter getPrinter() {
        return printer;
    }
    ...
    public String reporteReservasFecha() {
        ...
        try {
            ...
            for (Recurso recurso : recursos) {
                ...
                //ToDo: ver porqué esto no se usa
                List<Columna> defColumnas = new ArrayList<>();
            ...
            printer = new CSVWebFilePrinter(fileCSV, nombre);
    ...
    private void mostrarConfirmacionReporte() {
        addInfoMessage(sessionMBean.getTextos().get("mensaje_reporte_disponible"));
    }
    ...
    public void reporteAtencionPeriodo(ActionEvent e) {
        ...
        if (!hayErrores) {
            List<AtencionReporteDT> listAtencionReportDT = new ArrayList<>();
            if (this.todasLasEmpresas) {
                Empresa empresaActual = sessionMBean.getEmpresaActual();
                try {
                    for (Empresa emp : usuariosEmpresasEJB.consultarEmpresasPorOrganismo(sessionMBean.getOrganismoActual())) {
                        try {
    ...
    private List<AtencionReporteDT> obtenerDatosReporteAtencionPeriodo(Date fechaDesde, Date fechaHasta) {
        ...
        int asistencias = 0;
        int inasistencias = 0;
        List<AtencionReporteDT> listAtencionReport = new ArrayList<>();
    ...
    public void reporteTiemposAtencion() {
        ...
        if (!hayErrores) {
            List<AtencionLLamadaReporteDT> llamadas = new ArrayList<>();
            if (this.todasLasEmpresas) {
                Empresa empresaActual = sessionMBean.getEmpresaActual();
                try {
                    for (Empresa emp : usuariosEmpresasEJB.consultarEmpresasPorOrganismo(sessionMBean.getOrganismoActual())) {
    ...
    private List<List<TableCellValue>> armarContenido(Recurso recurso, List<ReservaDTO> reservas, List<AgrupacionDato> agrupaciones) {
        ...
        for (ReservaDTO reserva : reservas) {
            ...
            filaDatos.add(new TableCellValue(StringUtils.isNotBlank(reserva.getCodigoSeguridad()) ? reserva.getCodigoSeguridad():""));
            for (AgrupacionDato grupo : agrupaciones) {
                for (DatoASolicitar campo : grupo.getDatosASolicitar()) {
                    if (campo.getIncluirEnReporte()) {
            ...
            if (reserva.getAsistio() == null) {
                ...
                filaDatos.add(new TableCellValue(reserva.getAsistio() ? sessionMBean.getTextos().get("si") : sessionMBean.getTextos().get("no")));
            }
            ...
            if (reserva.getObservaciones() == null) {
                ...
                filaDatos.add(new TableCellValue(reserva.getObservaciones().trim()));
            }
