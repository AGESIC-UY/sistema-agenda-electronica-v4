...
/**
 * Esta clase es utilizada para permitir la autenticación anónima de usuarios
 * para la parte pública de la aplicación. Básicamente, se trata de autenticar a
 * un usuario "inventado" y temporal, de forma de poder registrarlo en una
 * empresa y así lograr que la aplicación pueda utilizar un EntityManager
 * asociado al esquema de dicha empresa.
 *
 * El código de usuario debe estar compuesto por un nombre y el identificador de
 * la empresa separados por una barra. Por ejemplo "usuario12345/3". El nombre
 * debería ser único o al menos no tener probabilidad de colisionar con el
 * nombre de otro usuario. La contraseña debe ser el MD5 mismo código de
 * usuario. No hay un problema de seguridad con esto ya que lo único que se
 * puede lograr es autenticar un usuario cuyo rol es "RA_AE_ANONIMO".
 *
 * @author spio
 *
 */
