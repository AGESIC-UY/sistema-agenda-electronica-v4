...
public class ServiciosREST {
    ...
    private String getReservasDocumento(ReservasPorDocumentoInput input) {
        try {
            List<Map<String, Object>> reservas = consultaEJB.consultarReservasPorTokenYDocumento(input.getToken(), input.getIdAgenda(),
    ...
    private String getReservasDocumentoFull(ReservasPorDocumentoInput input) {
        try {
            ...
            List<Map<String, Object>> reservas0 = consultaEJB.consultarReservasPorTokenYAgendaTramiteDocumento(input.getToken(), input.getIdAgenda(),
    ...
    private String getReservasVigentesDocumento(ReservasPorDocumentoInput input) {
        try {
            List<Map<String, Object>> reservas0 = consultaEJB.consultarReservasVigentesPorTokenYAgendaTramiteDocumento(input.getToken(), input.getIdAgenda(),
                    input.getIdRecurso(), input.getTipoDocumento(), input.getNumeroDocumento(), input.getCodigoTramite());
            JsonArray reservas1 = new JsonArray();
...
// Servicio agregado para ver. 2.3.5 17-04-2020 
    ...
    private String getReservasVigentesDocumentoEmpresa(ReservasPorEmpresaInput input) {
        try {
            ...
            boolean tokenValido = consultaEJB.validarTokenEmpresa(input.getToken(), Integer.parseInt(input.getIdEmpresa()));
            if (!tokenValido) {
                JsonObject jError = new JsonObject();
                jError.addProperty("resultado", "0");
            ...
            List<Map<String, Object>> reservas0 = consultaEJB.consultarReservasVigentesPorTokenYEmpresaTramiteDocumento(input.getToken(), input.getIdEmpresa(),
    ...
    private String getRecursosPorAgenda(RecursosPorAgendaInput input) {
        try {
            //Validar el token y la empresa
            boolean tokenValido = consultaEJB.validarTokenEmpresa(input.getToken(), input.getIdEmpresa());
            if (!tokenValido) {
                JsonObject jError = new JsonObject();
                jError.addProperty("resultado", "0");
                jError.addProperty("mensaje", "no_se_encuentra_el_token_especificado");
            ...
            Map<String, Object> agenda = consultaEJB.consultarRecursosPorAgenda(input.getIdEmpresa(), input.getIdAgenda(), input.getIdioma());
    ...
    private String getDisponibilidadesPorRecurso(DisponibilidadesPorRecursoInput input) {
        try {
            ...
            Map<String, Object> resp = consultaEJB.consultarDisponibilidadesPorRecurso(input.getIdEmpresa(), input.getIdAgenda(), input.getIdRecurso(),
                    input.getIdioma());
            //Transformar el resultado a un objeto JSon para devolver
    ...
    private String getDisponibilidadesPorRecursoTiempo(DisponibilidadesPorRecursoInput input) {
            ...
            try {
                ...
                httpRequest.login(falsoUsuario, password);
                //Obtener las disponibilidades
                Map<String, Object> resp = consultaEJB.consultarDisponibilidadesPorRecursoTiempo(input.getIdEmpresa(),  input.getIdAgenda(), input.getIdRecurso(), 
  ...
  // ======================================================================================================================================
    ...
    private String confirmarReserva(ConfirmarReservaInput input) {
        try {
            ...
            Reserva reserva = agendarReservasEJB.generarYConfirmarReserva(input.getIdEmpresa(), input.getIdAgenda(), input.getIdRecurso(), input.getIdDisponibilidad(),
                    input.getDatosReserva(), input.getIdTransaccionPadre(), input.getPasoTransaccionPadre(), null, input.getIdioma(), OrigenReserva.REST, new InformacionRequestDTO(httpRequest.getScheme(), httpRequest.getServerName(), "/sae", httpRequest.getServerPort()));
            Agenda agenda = agendarReservasEJB.consultarAgendaPorId(input.getIdAgenda());
            Recurso recurso = reserva.getDisponibilidades().get(0).getRecurso();
            Empresa empresa = agendarReservasEJB.obtenerEmpresaPorId(input.getIdEmpresa());
            ...
            Map<String, Object> datosEmpresa = consultaEJB.consultarDatosEmpresa(input.getIdEmpresa());
            ...
            try {
                if (input.getNotificar() == null || input.getNotificar() == true) {
                    agendarReservasEJB.modificarReservaNotificar(reserva.getId(), true);
                    comunicacionesEJB.enviarComunicacionesConfirmacion(empresa, linkCancelacion, linkModificacion, reserva, input.getIdioma(), formatoFecha, formatoHora);
                } else {
                    agendarReservasEJB.modificarReservaNotificar(reserva.getId(), false);
                }
    ...
    private String cancelarReserva(CancelarReservaInput input) {
        try {
            ...
            httpRequest.login(falsoUsuario, password);
            //Obtener la reserva
            Reserva reserva = agendarReservasEJB.consultarReservaPorId(input.getIdReserva());
            if (reserva == null) {
            ...
            //Cancelar la reserva
            Recurso recurso = reserva.getDisponibilidades().get(0).getRecurso();
            agendarReservasEJB.cancelarReserva(input.getIdEmpresa(), recurso.getAgenda().getId(), recurso.getId(), input.getIdReserva());
            ...
            try {
                ...
                if (reserva.getNotificar() == true) {
                    comunicacionesEJB.enviarComunicacionesCancelacion(empresa, reserva, input.getIdioma(), formatoFecha, formatoHora);
                }
            } catch (Exception ex) {
    ...
    private String reservaMultipleObtenerToken(ReservaMultipleObtenerTokenInput input) {
        try {
            ...
            Recurso recurso = agendarReservasEJB.consultarRecursoPorId(agenda, input.getIdRecurso());
            if (recurso == null) {
            ...
            TokenReserva tokenReserva = agendarReservasEJB.generarTokenReserva(input.getIdRecurso(), input.getDocumento(), input.getNombre(), input.getEmail(),
    ...
    private String reservaMultipleAnadirReserva(ReservaMultipleAnadirReservaInput input) {
        try {
            ...
            TokenReserva tokenReserva = agendarReservasEJB.obtenerTokenReserva(input.getTokenReserva());
            ...
            if(empresa!=null && empresa.getOrganismoId()!=null){
                sTiempoMaximo = configuracionEJB.getString("RESERVA_MULTIPLE_PENDIENTE_TIEMPO_MAXIMO",empresa.getOrganismoId());
            }
            ...
            //Generar y confirmar la reserva
            Reserva reserva = agendarReservasEJB.generarYConfirmarReserva(input.getIdEmpresa(), input.getIdAgenda(), input.getIdRecurso(), input.getIdDisponibilidad(),
                    input.getDatosReserva(), null, null, tokenReserva, input.getIdioma(), OrigenReserva.REST,  new InformacionRequestDTO(httpRequest.getScheme(), httpRequest.getServerName(), "/sae", httpRequest.getServerPort()));
            //Enviar la respuesta
            JsonObject jReserva = new JsonObject();
            jReserva.addProperty("resultado", "1");
            jReserva.addProperty("id", reserva.getId());
    ...
    private String reservaMultipleEliminarReserva(ReservaMultipleEliminarReservaInput input) {
        try {
            ...
            //Eliminar la reserva
            agendarReservasEJB.cancelarReservaMultiple(tokenReserva.getId(), input.getIdReserva());
            //Enviar la respuesta
            JsonObject jReserva = new JsonObject();
            jReserva.addProperty("resultado", "1");
            return jReserva.toString();
    ...
    private String reservaMultipleConfirmarReservas(ReservaMultipleConfirmarReservasInput input) {
        try {
            ...
            tokenReserva = agendarReservasEJB.confirmarReservasMultiples(input.getIdEmpresa(), tokenReserva.getId(), input.getIdTransaccionPadre(), input.getPasoTransaccionPadre(), false, new InformacionRequestDTO(httpRequest.getScheme(), httpRequest.getServerName(), "/sae", httpRequest.getServerPort()));
            ...
            try {
                if (input.getNotificar() == null || input.getNotificar() == true) {
                    agendarReservasEJB.modificarReservaMultipleNotificar(tokenReserva, true);
                    comunicacionesEJB.enviarComunicacionesConfirmacion(empresa, linkCancelacion, linkModificacion, tokenReserva, input.getIdioma(), formatoFecha, formatoHora);
                } else {
                    agendarReservasEJB.modificarReservaMultipleNotificar(tokenReserva, false);
                }
            } catch (Exception e) {
                //ignoro
            }
            //Armar la respuesta
            JsonArray jReservas = new JsonArray();
            List<Reserva> reservas = agendarReservasEJB.obtenerReservasMultiples(tokenReserva.getId(), false);
    ...
    private String reservaMultipleCancelarReservas(ReservaMultipleCancelarReservasInput input) {
        try {
            ...
            tokenReserva = agendarReservasEJB.cancelarReservasMultiples(tokenReserva.getId());
            JsonObject jResultado = new JsonObject();
            jResultado.addProperty("resultado", "1");
            return jResultado.toString();
        } catch (UserException uEx) {
    ...
    private String modificarReserva(ModificarReservaInput input) {
        try {
            ...
            Reserva reservaOriginal = agendarReservasEJB.consultarReservaPorId(input.getIdReserva());
            ...
            Reserva reservaNueva = agendarReservasEJB.modificarReserva(input.getIdEmpresa(), input.getIdAgenda(), input.getIdRecurso(), input.getIdReserva(),
                    input.getIdDisponibilidad(), null, input.getIdioma(), null, OrigenReserva.REST);
            ...
            try {
                if (reservaNueva.getNotificar() == true) {
                    comunicacionesEJB.enviarComunicacionesCancelacion(empresa, reservaOriginal, input.getIdioma(), formatoFecha, formatoHora);
                }
            } catch (Exception w) {
                //ignoro
            }
            ...
            try {
                if (reservaNueva.getNotificar() == true) {
                    comunicacionesEJB.enviarComunicacionesConfirmacion(empresa, linkCancelacion, linkModificacion, reservaNueva, input.getIdioma(), formatoFecha, formatoHora);
                }
            } catch (Exception w) {
                //ignoro
            }
...
//	}
  ...
  // ======================================================================================================================================
    ...
    @Path("/cambios-en-recurso")
    @Produces("application/json")
    public String cambiosEnRecursoGet(@QueryParam("token") String token, @QueryParam("idEmpresa") Integer idEmpresa, @QueryParam("idAgenda") Integer idAgenda, @QueryParam("idRecurso") Integer idRecurso,
            @QueryParam("fechaDesde") String fechaDesde, @QueryParam("fechaHasta") String fechaHasta) {
    ...
    private String obtenerCambios(CambiosEnRecursoInput input) {
        try {
            ...
            if (input.getToken() == null) {
                return jError("debe_especificar_el_token");
            }
            if (input.getIdEmpresa() == null) {
                return jError("debe_especificar_la_empresa");
            }
            ...
            if (input.getFechaDesde() != null && !input.getFechaDesde().trim().isEmpty()) {
                try {
                    fechaDesde = LocalDate.parse(input.getFechaDesde(), FORMATO_FECHA)
                        .atStartOfDay();
            ...
            if (input.getFechaHasta() != null && !input.getFechaHasta().trim().isEmpty()) {
                try {
                    fechaHasta = LocalDate.parse(input.getFechaDesde(), FORMATO_FECHA)
                        .atTime(23, 59, 59);
                } catch (DateTimeParseException ex) {
            ...
            List<RecursoDTO> registrosDeCambioEnRecurso = consultaEJB.consultarCambiosEnUnRecurso(input.getToken(), input.getIdEmpresa(),
    ...
    private String reservaMultipleAnadirReservaVacunacion(ReservaMultipleAnadirReservaInput input) {
        ...
        try {
            ...
            //Generar y confirmar la reserva
            Reserva reserva = agendarReservasEJB.generarYConfirmarReservasVacunacion(input.getIdEmpresa(), input.getIdAgenda(), input.getIdRecurso(), input.getIdDisponibilidad(),
                    input.getDatosReserva(), null, null, tokenReserva, input.getIdioma(), OrigenReserva.REST, fechaDos,input.getTipoDocReservaDos(),input.getTipoDosisReservaDos());
            //Enviar la respuesta
            JsonObject jReserva = new JsonObject();
            jReserva.addProperty("resultado", "1");
            jReserva.addProperty("id", reserva.getId());
    ...
    private String reservaMultipleEliminarReservasVacunacion(ReservaMultipleEliminarReservaInput input) {
        try {
            ...
            //Eliminar la reserva
            agendarReservasEJB.cancelarReservasParesMultiple(tokenReserva.getId(), input.getIdReserva());
            //Enviar la respuesta
            JsonObject jReserva = new JsonObject();
            jReserva.addProperty("resultado", "1");
            return jReserva.toString();
    ...
    private String cancelarReservaVacunacion(CancelarReservaInput input) {
        try {
            ...
            Reserva reserva2 = agendarReservasEJB.consultarReservaPorId(reserva.getReservaHija().getId());
            if (reserva2 == null) {
                throw new UserException("no_se_encuentra_la_reserva_o_ya_fue_cancelada");
            }
            //Verificar el código de cancelación
            ...
            //Cancelar la reserva
            Recurso recurso = reserva.getDisponibilidades().get(0).getRecurso();
            agendarReservasEJB.cancelarReservaVacunacion(input.getIdEmpresa(), recurso.getAgenda().getId(), recurso.getId(), input.getIdReserva(),reserva2.getId(),false);
            //Enviar las comunicaciones   
            try {
                Empresa empresa = agendarReservasEJB.obtenerEmpresaPorId(input.getIdEmpresa());
                //ToDo: quitar esto si se obtiene la empresa
                ...
                if (reserva2.getNotificar() == true) {
                    comunicacionesEJB.enviarComunicacionesCancelacion(empresa, reserva2, input.getIdioma(), formatoFecha, formatoHora);
                }
