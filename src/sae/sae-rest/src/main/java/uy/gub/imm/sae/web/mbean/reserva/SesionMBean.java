...
public class SesionMBean extends BaseMBean {
    ...
    private static final Logger LOGGER = Logger.getLogger(SesionMBean.class);
    ...
    private Boolean bMostrarFechaActual;

    //Para la reserva múltiple
    private TokenReserva tokenReserva;
    
    public static final String URL_BASE_TO_FORWARD_IDURUGUAY = "/iduruguayRedirect/iduredirect.xhtml";
    ...
    public void init() {
        ...
        if (empresaActual != null && empresaActual.getOrganismoId() != null) {
            bMostrarFechaActual = configuracionEJB.getBoolean("MOSTRAR_FECHA_ACTUAL",empresaActual.getOrganismoId());
            googleAnalytics = configuracionEJB.getString("GOOGLE_ANALYTICS",empresaActual.getOrganismoId());
        }
    ...
    //***************************************** IDURUGUAY *****************************************************************************************************
    //**************************************************************************************************************************************************************
    private String usuarioIduruguay = null;

    private String ssoBaseUrl = null;

    public String getUsuarioIduruguay() {
        return usuarioIduruguay;
    }

    public void setUsuarioIduruguay(String usuarioIduruguay) {
        this.usuarioIduruguay = usuarioIduruguay;
    }
    
    public String cerrarSesionIDURUGUAY() {
        try {
            Context initContext = new InitialContext();
            String urlLogout = (String)initContext.lookup("java:global/iduruguayhttphandler/ssoRedirectUrlSaePublico");
    ...
    public void setDiaSeleccionado(Date diaSeleccionado) {
        ...
        } else if (this.diaSeleccionado == null || !diaSeleccionado.equals(this.diaSeleccionado)) {
     ...
     * inicialmente se posiciona en el mes correspondiente a la fecha inicial de
     * la la ventana del calendario de la agenda, pero luego mediante el
     * setCurrentDate el componente grafico actualiza esta fecha según nos
     * movamos en los meses/años del calendario. No confundir con el dia
    ...
    private Map<String, String> preguntasCaptcha = new HashMap<>();

    public void cargarTextos() {
        try {
            textos = agendarReservasEJB.consultarTextos(idiomaActual);
        } catch (ApplicationException ex) {
            LOGGER.error("Error al cargar los textos", ex);
            textos = new SofisHashMap();
        }
        try {
            preguntasCaptcha = agendarReservasEJB.consultarPreguntasCaptcha(idiomaActual);
        } catch (ApplicationException ex) {
            LOGGER.error("Error al cargar las preguntas de captcha", ex);
            preguntasCaptcha = new HashMap<>();
        }
    }
